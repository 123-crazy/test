...
function _loadRichTextEditor() {
    return new Promise( ( resolve ) => {
        if( browserUtils.isIE || browserUtils.isSafari ) { // Ckeditor4 if IE browser
            isIE = true;
            _loadCkeditor4().then(
                function( ) {
                    resolve();
        ...
        function( ) {
            if( isIE ) {
                ...
                _loadExtraPluginsForCkeditor5( config.extraPlugins ).then( loadedPlugins => { // Dynamic loading of extra plugins
                    ...
                    CKEDITOR5.InlineEditor.create( document.querySelector( '#' + elementId ), config ).then( editor5 => {
                        editor5 = new AWCkeditor5( editor5 );
                        if( appCtxSvc.getCtx( 'Ac0ConvCtx' ).editConvCtx ) {
                            editor5._instance.isReadOnly = true;
                        }
