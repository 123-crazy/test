...
export function init( viewerContainer ) {
    ...
    if( container.children.playControl ) {
        ...
        playButton.innerHTML = resources.imgPlay ? resources.imgPlay : svg.replace( /xyz/, 'M9,7V17L17,12z' );
        ...
        pauseButton.innerHTML = resources.imgPause ? resources.imgPause : svg.replace( /xyz/, 'M8,8V16H11V8H7.5M13,8V16H16V8H12.5' );
 ...
 */
export function setCurrentTime( t, seek ) {
    currentTime = t;
 ...
 */
export function setTicks( array ) {
    ticks = array;
    currentTick = null;
    if( ! ticks ) {
 ...
 */
export function playInterval( start, end, callback ) {
 ...
 */
export function setPlayChangeCallback( callback ) {
 ...
 * @param {Resource} value - the resource, can be icon, image, or svg
 */
export function addResource( name, value ) {
    resources[ name ] = ! value ? null :
        value.replace( /#464646/g, 'white' ).replace( /class="aw-theme-iconOutline"/g, '');
}
