...
export let evaluateObjsBasedOnConfiguredRevRuleForPaste = function( primaryObject, secondaryObject, relationInfo ) {
    ...
    if( primaryObject && primaryObject.modelType && secondaryObject ) {
        if( primaryObject.modelType.typeHierarchyArray.includes( 'Folder' ) && relationInfo ===
            '' ) { // Paste triggered through drag and drop to a Folder ; evaluation not done through addObjectContext  , hence evaluation required
 ...
 */
let getWSOSubtypesForCreatedObjs = ( inputObjects ) => {
    let finalObjs = [];
