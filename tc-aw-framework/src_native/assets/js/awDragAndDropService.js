...
var _deselectAllAndPasteSourceFiles = function( targetElement, targetVMO, sourceFiles, dragAndDropParams, xrtTableUid ) {
    if( sourceFiles && sourceFiles.length > 0 ) {
        ...
        let datasetRelationTypes = null;
        if( xrtTableUid && dragAndDropParams && dragAndDropParams.dataProvider ) {
            datasetRelationTypes = _getAllValidRelationTypesForDataset( dragAndDropParams.dataProvider );
        }
        if( !validSourceTypes || _.isEmpty( validSourceTypes ) ) {
            if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMO.modelType ) ) {
                adapterSvc.getAdaptedObjects( [ targetVMO ] ).then( function( adaptedObjs ) {
                    validSourceTypes = pasteService.getObjectValidSourceTypes( adaptedObjs[ 0 ] );
                    _pasteSourceFiles( targetVMO, sourceFiles, dragAndDropParams, validSourceTypes, datasetRelationTypes );
                } );
            } else {
 ...
 * if so, change drag effect to indicate if it is OK to drop on that 'target'.
 *
 * @param {Object} dataProvider - dataProvider get from drag and drop parameters.
 * @return {Object} validRelationTypes - get the relation type which source is Dataset
 */
const _getAllValidRelationTypesForDataset = ( dataProvider ) => {
    let validRelationTypesForDataset = [];
    if( dataProvider && dataProvider.validSourceTypes ) {
        var objectSetSources = dataProvider.validSourceTypes.split( ',' );
        _.forEach( objectSetSources, function( source ) {
            var relationSources = source.split( '.' );
            var sourceType = relationSources[ 1 ];
            if( sourceType === TYPE_NAME_DATASET && relationSources[ 0 ] ) {
                validRelationTypesForDataset.push( relationSources[ 0 ] );
            }
        } );
    }
    return validRelationTypesForDataset;
};
...
const createPasteInput = ( pasteRelation2SourceObjectsMap, targetVMO, datasetRelationTypes ) => {
    let pasteInput = [];
    _.forEach( pasteRelation2SourceObjectsMap, function( value, key ) {
        ...
        if( datasetRelationTypes ) {
            //add all valid source types get from dataprovder
            curr.datasetRelationTypes = datasetRelationTypes;
        }
...
const executeFileDrop = ( targetVMO, callbackAPIs, pasteRelation2SourceObjectsMap, datasetRelationTypes ) => {
    _scheduleSelectTarget( targetVMO, callbackAPIs );

    /**
     * Paste 'sources' to 'target' for each unique 'relation' type.
     */
    var pasteInput = createPasteInput( pasteRelation2SourceObjectsMap, targetVMO, datasetRelationTypes );
 ...
 * @param {Object} datasetRelationTypes - all dataset drop types get from dataprovider.
 */
var _pasteSourceFiles = function( targetVMO, sourceFiles, dragAndDropParams, validSourceTypes, datasetRelationTypes ) {
    /**
    ...
    if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMO.modelType ) ) {
        adapterSvc.getAdaptedObjects( [ targetVMO ] ).then( function( adaptedObjs ) {
            executeFileDrop( adaptedObjs[ 0 ], dragAndDropParams.callbackAPIs, pasteRelation2SourceObjectsMap, datasetRelationTypes );
        } );
    } else {
        executeFileDrop( targetVMO, dragAndDropParams.callbackAPIs, pasteRelation2SourceObjectsMap, datasetRelationTypes );
    }
};
