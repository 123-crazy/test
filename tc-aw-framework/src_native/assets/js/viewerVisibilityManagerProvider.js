...
export default class ViewerVisibilityManager extends ViewerVisibilityCoreManager {
    constructor( viewerCtxNamespace, viewerView, viewerContextData ) {
        assert( viewerContextData, 'Viewer context data can not be null' );
        super();
        var self = this;
        this.viewerCtxNamespace = viewerCtxNamespace;
        ...
        this.setUpVisibilityListener( viewerContextData, viewerView, viewerCtxNamespace );
    }
     ...
     * @param {Object} viewerCtxNamespace Viewer context name space
     */
    setUpVisibilityListener( viewerContextData, viewerView, viewerCtxNamespace ) {
        let self = this;
        /**
        ...
        this.viewerVisibilityListener = {
            ...
            updateVisibleState: function( occurrences ) {
                if( occurrences.length === 0 ) {
                    occurrences.push( self.viewerContextData.getViewerCtxSvc().createViewerOccurance( '' ) );
        ...
        viewerContextData.getViewerCtxSvc().updateViewerApplicationContext( viewerCtxNamespace,
            viewerContextData.getViewerCtxSvc().VIEWER_INVISIBLE_CSID_TOKEN, this.invisibleCsids );
        viewerContextData.getViewerCtxSvc().updateViewerApplicationContext( viewerCtxNamespace,
            viewerContextData.getViewerCtxSvc().VIEWER_INVISIBLE_EXCEPTION_CSID_TOKEN, this.invisibleExceptionCsids );
    }
    /**
    ...
    toggleProductViewerVisibility( csidChain ) {
        ...
        this.viewerContextData.getViewerCtxSvc().updateViewerApplicationContext( this.viewerCtxNamespace,
            this.viewerContextData.getViewerCtxSvc().VIEWER_INVISIBLE_CSID_TOKEN, this.invisibleCsids );

        this.viewerContextData.getViewerCtxSvc().updateViewerApplicationContext( this.viewerCtxNamespace,
            this.viewerContextData.getViewerCtxSvc().VIEWER_INVISIBLE_EXCEPTION_CSID_TOKEN,
        ...
        _.forEach( csidChains, function( csidChain ) {
            ...
            if( partitionCsidChains && Array.isArray( partitionCsidChains ) && partitionCsidChains.length > 0 ) {
                if( !_.includes( partitionCsidChains, csidChain ) ) {
                    occ = this.viewerContextData.getViewerCtxSvc().createViewerOccurance( csidChain );
