...
function setLanguageDataOnScope( $scope, updateLocalStorage ) {
    $scope.$evalAsync( function() {
        ...
        localeSvc.getTextPromise( 'LoginLocale' ).then( function( textBundle ) {
            ...
            for( const localeKey in _localTextBundle ) {
...
export const setScope = function( $scope ) {
    ...
    $scope.hideLinkPopUp = function( event ) {
        ...
        // const parent = $( event.target ).closest( '.aw-layout-popup.aw-layout-popupOverlay' )[0];
    ...
    $scope.login = function() {
        ...
        sessionService.signIn( $scope.userName, $scope.password, $scope.group, $scope.role, localeSvc.getLocale(), getCurrentDiscriminator() ).then( function( responseData ) {
