 ...
 */
 ...
 import replaceDatasetService from 'js/Awp0ReplaceDatasetService';
  ...
  *            information.
  *
 * @param {StringArray} datasetRelationsFromDD - An array of relations types used when this method is called from drag and drop event.
 *
 ...
 function _createDatasetsAndRelate( targetObject, getDatasetTypesResponse, datasetInfos, sourceFiles, fileNames,
    relationType, fileInfos, datasetRelationsFromDD ) {
                         ...
                         for( var ii = 0; ii < extentionTypeInfo.length && !datasetType; ii++ ) {
                             ...
                             if( currExtTypeInfo.fileExtension === extension ) {
                                 ...
                                 for( var jj = 0; jj < currExtTypeInfo.datasetTypesWithDefaultRelInfo.length &&
                                    ...
                                    var defaultRelation = _.get( dataSetTypeInfo, 'defaultRelationInfo.name' );
                                     ...
                                     if( dsType ) {
                                        ...
                                        //re-write the relatonType when we have default relation type and it exist in the relation type list get from drag and drop event
                                        if( defaultRelation.length > 0 && Array.isArray( datasetRelationsFromDD ) && datasetRelationsFromDD.includes( defaultRelation ) ) {
 ...
 function _duplicateFilesExist( queryForFileExistenceResponse, targetObject, fileNames, fileNameToSourceFileMap ) {
     ...
     if ( queryForFileExistenceResponse !== undefined ) {
         ...
         if ( fileNamesToReplace.length > 0 ) {
             ...
             {
                 addClass: buttonNotification,
                 text: localTextBundle.replaceDataset,
                 onClick: function( $noty ) {
                     $noty.close();
                     deferred.resolve( fileNamesToCreate );
                     var promiseReplace = replaceDatasetService.replaceDataset( targetObject, fileNameToDataset );
                     if ( promiseReplace ) {
  ...
  */
 function prepareInputForFileExistenceSOA( pasteFilesInput ) {
     var inputForSOA = [];
 ...
 export let pasteFiles = function( pasteFilesInput ) {
     ...
     var inputForSoa = prepareInputForFileExistenceSOA( pasteFilesInput );
     ...
     soaSvc.postUnchecked( namespace, 'queryForFileExistence', inputForSoa ).then( function( response ) {
         ...
         _.forEach( pasteFilesInput, function( curr ) {
             ...
             var sourceFiles = curr.sourceObjects;
             var datasetRelationsFromDD = null;
             if ( curr.datasetRelationTypes ) {
                 datasetRelationsFromDD = curr.datasetRelationTypes;
             }
             ...
             if ( promiseReplace ) {
                 promiseReplace.then( function( fileNamesToCreate ) {
                     if ( fileNamesToCreate.length > 0 ) {
                         var fileInfos = [];
                         return _getDatasetTypes( targetObject, fileNamesToCreate ).then(
                             function( getDatasetTypesResponse ) {
                                 var sourceFileToCreate = [];
                                 _.forEach( fileNamesToCreate, function( fileName ) {
                                     sourceFileToCreate.push( fileNameToSourceFileMap[fileName] );
                                 } );
                                 return _createDatasetsAndRelate( targetObject, getDatasetTypesResponse, datasetInfos, sourceFileToCreate,
                                    fileNamesToCreate, relationType, fileInfos, datasetRelationsFromDD );
