 ...
 * @module js/ImgViewer
 */
import gifReader from 'js/GifReader';
import videoReader from 'js/VideoReader';
import playControl from 'js/PlayControl';
...
export function setImage( url ) {
    if( !url || url === loadingUrl ) {
        url || playControl.setPlayable( null );
        return;
    }
...
export let setTicks = function( array ) {
    playControl.setTicks( array );
};
export let getTicks = function() {
    return playControl.getTicks();
};
export let getDuration = function() {
    return playable ? playable.duration : undefined;
};
export let playInterval = function( start, end, callback ) {
    playControl.playInterval( start, end, callback );
};
export let setPlayChangeCallback = function( callback ) {
    playControl.setPlayChangeCallback( callback );
};
export let addResource = function( name, value ) {
    playControl.addResource( name, value );
};
