 ...
 */
export let getAssignQUsSoaInp = function( selectedObjs, mSelectedQProfiles, pSelectedQProfiles ) {
    var soaInputToPass = [];
    var assignQUInpts = {};
    var qUnits = [];
    var qProfiles = [];
    ...
    if ( pSelectedQProfiles && pSelectedQProfiles.type === 'Tq0QualificationProfile' ) {
 ...
 * @param {string} seperatorCode seperator code for seperating the date month and year
 */
 export let getCurrentDate = function( data, seperatorCode ) {
    var today = new Date();
    ...
    if ( dd < 10 ) { dd = '0' + dd; }
    if ( mm < 10 ) { mm = '0' + mm; }
    var dateStr = mm + sp + dd + sp + yyyy;
    data.date.dbValue = dateStr + ' 23:59:59';

    //// This block of code is calling the grid selection event and performing removal of Qualification Records.
    eventBus.subscribe( 'qualificationRecordsTable.gridSelection', function( eventData ) {
        appCtxSvc.ctx.selectionArr = eventData.selectedObjects;
        if ( !eventData.notResetSelectionArrForMessage ) {
            appCtxSvc.ctx.selectionArrForMessage = eventData.selectedObjects;
        }
    } );
};

// /**
//  * Method to set selections in table
//  *
//  *
//  */
...
/** This function creates the input for performSearchViewModel4 SOA and calls it.
 * @param {Object} - Selected Object QP/QU
 ...
 */
 saveEditHandler.isDirty = function( dataSource, data ) {
...
export let editStateChangeForRenewalSchedule = function( data ) {
    //appCtxSvc.ctx.editInProgress = true;
        ...
        vecNameVal: [
            {
                ...
                values: [
                    String( data.AllowEarlyRelease.uiValue )
                ]
            },
            {
            ...
            },
            {
                ...
                values: [
                    String( data.updateQualRecords.dbValue )
                ]
            }
        ]
    };
 ...
 * @param {*} ctx
 */
export let  displayReleaseSchedule = function( data, ctx ) {
    ctx = appCtxSvc.ctx;
    ...
    if( ctx.selected.props.tq0ImmediateRelease.dbValue === true && ctx.NONE._editing === false ) {
        releaseScheduleLabelUiVal = data.i18n.tq0WhenApproved;
    } else if( ctx.selected.props.tq0ReleaseAfterDays.dbValues[0] !== null && ctx.NONE._editing === false ) {
        releaseScheduleLabelUiVal = data.i18n.tq0DelaybyDays;
        ...
        releaseScheduleLabelUiVal = data.i18n.tq0DelaybyDate;
