...
export let createTreeCellCommandElement = ( column, vmo, tableElem ) => {
    ...
    let cellContent = awSPLMTableCellRendererFactory.createTreeCellCommandElement( column, vmo, tableElem );
...
export let updateWhatIfProperties = () => {
    if( !isTCVersion132OrLater() ) {
        ...
        if( viewModel ) {
            let vmNodes = viewModel.dataProviders.scheduleNavigationTreeDataProvider.viewModelCollection.loadedVMObjects;
