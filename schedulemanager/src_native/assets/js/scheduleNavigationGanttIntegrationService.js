    ...
    if( appCtxSvc.ctx.showGanttChart && !appCtxSvc.ctx.scheduleNavigationCtx.isGanttInitialized ) {
        ...
        if( rootTreeNode ) {
            try {
                ...
                if( viewModel ) {
                    let selectedObjectUids = _.map( viewModel.dataProviders.scheduleNavigationTreeDataProvider.getSelectedObjects(), 'uid' );
...
var resetRowHeight = () => {
    let viewModel = viewModelService.getViewModelUsingElement( schNavTreeUtils.getScheduleNavigationTreeTableElement() );
    let rowHeight = appCtxSvc.ctx.layout === 'compact' ? _t.Const.HEIGHT_COMPACT_ROW : _t.Const.HEIGHT_ROW;
    if( viewModel ) {
...
export let setBaselineToView = ( loadedBaselines ) => {
    ...
    appCtxSvc.updateCtx( 'showGanttTaskProperties', false );
