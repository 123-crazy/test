...
*/
export let initializePanelData = function (data) {
    let panelData = {};
...
export let updateDataOnPanel = function (data) {
    if ( data.daysBeforeFinishDate.dbValue === null ) {
        data.daysBeforeFinishDate.dbValue = 0;
        data.daysBeforeFinishDate.uiValue = 0 + '';
    }
    ...
    let dataToUpdateOnPanel = {
        ...
        isSelectedVMOForSubscription: data.isSelectedVMOForSubscription
    };
...
var updatePanelForUpdate = function (data, subscriptionToUpdate, dataToUpdateOnPanel) {
    ...
    let eventTypeObject = cdm.getObject(subscriptionToUpdate.props.event_type.dbValue);
    ...
    if (recipientUids && recipientUids.length > 0) {
        dms.loadObjects(recipientUids).then(function () {
            ...
            addObject(data, recipient, data.dataProviders.followers_provider);
        });
    }
...
/**
 * Add selected objects from "Add Resource" panel to main panel.
 *
 * @param {object} data - The CreateNotification view model data
 * @param {object} selectedTab - The User, Resource Pool or Discipline
*/
export let addObjectsToFollowersDataProvider = function (data, selectedTab) {
    let vmosToAddInFollowersList = [];
    //Get objects from Resource panels
    if (selectedTab === 'UserSub') {
        if (data.dataProviders.userPerformSearch.selectedObjects.length) {
            for (let i = 0; i < data.dataProviders.userPerformSearch.selectedObjects.length; i++) {
                let userUid = data.dataProviders.userPerformSearch.selectedObjects[i].props.user.dbValue;
                vmosToAddInFollowersList.push(cdm.getObject(userUid));
            }
        }
    } else if (selectedTab === 'Saw1AddResourcePoolToSchedule') {
        if (data.dataProviders.getResourcePool.selectedObjects.length) {
            for (let i = 0; i < data.dataProviders.getResourcePool.selectedObjects.length; i++) {
                let resPoolVMO = data.dataProviders.getResourcePool.selectedObjects[i];
                vmosToAddInFollowersList.push(resPoolVMO);
            }
        }
    } else if (selectedTab === 'Saw1AddDisciplinesToSchedule') {
        if (data.dataProviders.getDisciplines.selectedObjects.length) {
            for (let i = 0; i < data.dataProviders.getDisciplines.selectedObjects.length; i++) {
                let disVMOVMO = data.dataProviders.getDisciplines.selectedObjects[i];
                vmosToAddInFollowersList.push(disVMOVMO);
            }
        }
    }

    // Add object to followers data provider.
    addObject(data, vmosToAddInFollowersList, data.dataProviders.followers_provider);

    //Navigate back to Create Panel.
    let context = {
...
export let addObject = function (data, newObjects, dataProviders) {
    if (newObjects) {
        if (newObjects.length > 1 && newObjects[0].type === 'ImanSubscription' && newObjects[0].props && newObjects[0].props.expiration_date ) {
            newObjects.sort((a, b) => {
                let avalue = a.props.expiration_date.dbValues[0] === null;
                let bvalue = b.props.expiration_date.dbValues[0] === null;
                if ( avalue === bvalue ) {
                   return 0;
                }
                return avalue ? -1 : 1;
        ...
        for (let i = 0; i < newObjects.length; i++) {
            ...
            for (let j = 0; j < dataProviders.viewModelCollection.loadedVMObjects.length; j++) {
...
export let removeObject = function (removedObjects, dataProviders) {
    if (removedObjects) {
        ...
        let tempObject = [];
        for (let i = 0; i < dataProviders.viewModelCollection.loadedVMObjects.length; i++) {
        ...
        for (let j = 0; j < tempObject.length; j++) {
            for (let k = 0; k < removedObjects.length; k++) {
...
export let initPanelForMultipleEvents = function (data, userConfiguredEventList) {
    let dataToUpdateOnPanel = {
        eventType: data.eventType,
        myEventsCheckbox: data.myEventsCheckbox
    };
    ...
    if (userConfiguredEventList && userConfiguredEventList.length > 0) {
        ...
        for (let i = 0; i < userConfiguredEventList.length; i++) {
            eventName = eventName + userConfiguredEventList[i].props.eventtype_id.dbValues[0];
            eventDisplayName = eventDisplayName + userConfiguredEventList[i].props.object_string.dbValues[0];
...
export let updatePanelForUncheckMultiEventCheckBox = function (data) {
    ...
    if (displayStrings && displayStrings.length > 0) {
        let eventType = data.eventType.dbOriginalValue;
...
export let getAvailableEvents = function (data) {
    ...
    for (let i = 0; i < configuredObjects.length; i++) {
        configuredEventNames.push(configuredObjects[i].props.eventtype_id.dbValue);
    }
...
exports = {
    ...
    addObjectsToFollowersDataProvider,
    initializePanelData,
    getAvailableEvents,
 ...
 *
 * @member Saw1NotificationService
 * @memberof NgServices
 */
app.factory('Saw1NotificationService', () => exports);
