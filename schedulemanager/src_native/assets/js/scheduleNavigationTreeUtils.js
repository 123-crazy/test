...
export let getScheduleNavigationTreeTableElement = () => {
    ...
    return getFirstMatchingElement( tables, 'gridid', 'scheduleNavigationTree' );
};
...
export let getScheduleNavigationGanttElement = () => {
    ...
    return  getFirstMatchingElement( includeElements, 'view-id', 'scheduleNavigationGantt' );
};
...
export let getTreeTableHeight = () => {
    let tableHeight = 0;
    let scheduleNavigationTreeTable = getScheduleNavigationTreeTableElement();

    if( scheduleNavigationTreeTable ) {
        tableHeight = _t.Trv( scheduleNavigationTreeTable ).getTableContainerElementFromTable().clientHeight;
    }
    return tableHeight;
};
...
export let getRootTreeNode = () => {
    ...
    if( viewModel ) {
        rootTreeNode = viewModel.dataProviders.scheduleNavigationTreeDataProvider.viewModelCollection.loadedVMObjects[ 0 ];
    }
    return rootTreeNode;
};
