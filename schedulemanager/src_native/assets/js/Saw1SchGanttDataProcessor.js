...
export default class Saw1SchGanttDataProcessor extends GanttDataProcessor {
    ...
    getGanttHeight() {
        let height = 0;
        if( appCtxSvc.ctx.locationContext && appCtxSvc.ctx.locationContext[ 'ActiveWorkspace:SubLocation' ] === 'com.siemens.splm.client.schmgt:ScheduleNavigationSubLocation' ) {
            height = schNavTreeUtils.getTreeTableHeight() + 2;
        } else {
            ...
            var columnHeight = 30;
            height = subLocHeight - xrtTabHeight - toolbarHeight - columnHeight;
            if( height < 200 ) {
                height = 200;
            }
        }
    ...
    getVisibleTasks() {
        ...
        loadedVMObjects.forEach( ( loadedVMO ) => {
            let uid = loadedVMO.uid;
            if( uid ) {
