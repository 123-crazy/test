...
export const dragOverTreeNode = ( dragAndDropParams ) => {
    ...
    let isDnDEnabled = appCtxSvc.ctx.scheduleNavigationCtx && appCtxSvc.ctx.scheduleNavigationCtx.isStructureEditSupported;
    if( isDnDEnabled && targetObject && cmm.isInstanceOf( 'ScheduleTask', targetObject.modelType ) && dragDataCache.draggedObjects.length === 1 ) {
        ...
        if( areSourceTypesValid && dragDataCache.draggedObjUids.indexOf( targetObject.uid ) <= -1 ) {
            ...
            if( currentTargetCoordinate > rowTopEdge && currentTargetCoordinate < rowTopEdge + domRectForTarget.height ) {
                ...
                let splmTable = _t.util.closestElement( dragAndDropParams.targetElement, '.aw-splm-table' );
...
export let updateScheduleTreeNodePosition = ( treeDataProvider, moveTaskInfo ) => {
    if( treeDataProvider.viewModelCollection && treeDataProvider.viewModelCollection.loadedVMObjects && treeDataProvider.viewModelCollection.loadedVMObjects.length > 0 ) {
        ...
        removeScheduleFromOldParent( loadedVMObjects, taskTreeNode, taskTreeNodeIndex, oldParentNodeTreeNodeIndex, updateScheduleTreeNodeList );
        addScheduleToNewParent( loadedVMObjects, updateScheduleTreeNodeList, newParentTreeNodeIndex, taskTreeNode, prevSiblingTask );
...
/**
 * Remove schedule and its children from its old parent tree node
 *
 ...
 */
let removeScheduleFromOldParent = ( loadedVMObjects, taskTreeNode, taskTreeNodeIndex, oldParentNodeTreeNodeIndex, updateScheduleTreeNodeList ) => {
...
/**
 * Add schedule and its children to its new parent tree node
 *
 * @param {Array} loadedVMObjects view model collection
 * @param {Array} updateScheduleTreeNodeList updated schedule object list
 ...
 */
let addScheduleToNewParent = ( loadedVMObjects, updateScheduleTreeNodeList, newParentTreeNodeIndex, taskTreeNode, prevSiblingTask ) => {
...
app.factory( 'scheduleNavigationTreeDragDropService', () => exports );
