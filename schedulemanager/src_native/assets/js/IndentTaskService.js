 ...
 */
export let getIndentValidation = function( data ) {
    ...
    let taskNotToBeUpdated = userListService.getTasksNotToBeUpdated( selectedTasks, data, true );
    ...
    selectedTasks.forEach( function( seletcedTask ) {
        ...
        if( !parentTask ) {
            message = prepareIndentTaskErrorMessage( data, taskNotToBeUpdated, selectedTasks );
...
export let getParentTaskObject = function() {
    let viewModel = viewModelService.getViewModelUsingElement( schNavTreeUtils.getScheduleNavigationTreeTableElement() );
    let selectedTasks = [];
    if( viewModel ) {
        selectedTasks = viewModel.dataProviders.scheduleNavigationTreeDataProvider.getSelectedObjects();
...
export let performIndentAction = function( treeDataProvider, eventData ) {
    ...
    if( selectedObjects && selectedObjects.length > 0 ) {
        removeSelectedNodesFromTree( selectedObjects, loadedVMObjects, treeDataProvider, selectedTreeNodesMap, oldParentUid );
        ...
        if( newParentTreeNode.isLeaf ) {
            ...
            addSelectedNodesToNewParentTreeNode( newParentTreeNode, selectedObjects, selectedTreeNodesMap, newParentTreeNodeIndex, loadedVMObjects );
        } else {
