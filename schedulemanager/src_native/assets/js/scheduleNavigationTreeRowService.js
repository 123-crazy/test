...
import schTreeService from 'js/scheduleNavigationTreeService';
...
export let subscribeEvents = () => {
    ...
    eventSubscriptions.push( eventBus.subscribe( 'scheduleNavigationTree.nodesAdded', eventData => {
        _.forEach( eventData.addedNodes, function( vmo ) {
            let index = getTaskIndexFromLoadedVMO( vmo.uid );
            appCtxSvc.ctx.scheduleNavigationCtx.treeNodeUids.splice( index, 0, vmo.uid );
        } );
    ...
    eventSubscriptions.push( eventBus.subscribe( 'scheduleNavigationTree.dependenciesAdded', eventData => {
        processCreatedDependency( eventData.eventInfo );
        refreshDependenciesDisplayValues( );
        eventBus.publish( 'scheduleNavigationTree.plTable.clientRefresh' );
    } ) );
    ...
    eventSubscriptions.push( eventBus.subscribe( 'scheduleNavigationTree.dependenciesDeleted', eventData => {
        processDeletedDependency( eventData.deletedDepInfos );
        refreshDependenciesDisplayValues( );
        eventBus.publish( 'scheduleNavigationTree.plTable.clientRefresh' );
    } ) );
    ...
    eventSubscriptions.push( eventBus.subscribe( 'scheduleNavigationTree.tasksReordered', eventData => {
        ...
        let loadedVMObjects = viewModel.dataProviders.scheduleNavigationTreeDataProvider.viewModelCollection.loadedVMObjects;
...
let getTaskIndexFromLoadedVMO = function( taskUid ) {
    let viewModel = viewModelService.getViewModelUsingElement( schNavTreeUtils.getScheduleNavigationTreeTableElement() );
    let viewModelCollection = viewModel.dataProviders.scheduleNavigationTreeDataProvider.viewModelCollection;
    return viewModelCollection.findViewModelObjectById( taskUid );
};
    ...
    try {
        ...
        if( index === -1 ) {
            ...
            if( propertyName === 'saw1Successors' ) {
                ...
                stringNewValue = scheduleTaskVMO.props.saw1Successors.dbValues[0];
            }

            if( propertyName === 'saw1Predecessors' ) {
                depInfo = schNavigationDepCacheService.getTaskPredDependencies( scheduleTaskVMO.uid );
                stringNewValue = scheduleTaskVMO.props.saw1Predecessors.dbValues[0];
            }
            var depToRemove = [];
            ...
            var scheduleVMO = appCtxSvc.ctx.locationContext.modelObject;
 ...
 * @param {data} data - Data
*/
export let refreshDependenciesDisplayValues = function( ) {
    regenerateDependencyIds();
    let viewModel = viewModelService.getViewModelUsingElement( schNavTreeUtils.getScheduleNavigationTreeTableElement() );
    updateDependencyValues( viewModel.dataProviders.scheduleNavigationTreeDataProvider );
};
...
/**
 * @memberof NgServices
 * @member scheduleNavigationTreeRowService
 */
app.factory( 'scheduleNavigationTreeRowService', () => exports );
