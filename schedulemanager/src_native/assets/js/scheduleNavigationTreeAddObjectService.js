    ...
    uidsToAdd.forEach( uidToAdd => {
        ...
        if( !addTaskInfo.parentNode.isExpanded || addTaskInfo.parentNode.isLeaf ) {
            ...
            if( addTaskInfo.parentNode.__expandState ) {
                let deletedData = { deletedUids : [], removedNodes : [] };
        ...
        if( childNode ) {
            ...
            eventBus.publish( 'scheduleNavigationTree.nodesAdded', { addedNodes: [ childNode ] } );
        }
    } );
 ...
 */
export let addNewDependencies = ( dependencies ) => {
    ...
    if( !_.isEmpty( eventInfo ) ) {
        eventBus.publish( 'scheduleNavigationTree.dependenciesAdded', { eventInfo } );
    }
};
