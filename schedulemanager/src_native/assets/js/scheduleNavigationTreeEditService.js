...
export let addCreatedObjects = ( data, eventMap ) => {
    ...
    if( !_.isEmpty( dependencies ) ) {
        schNavTreeNodeAddService.addNewDependencies( dependencies );
    }
};
...
export let removeDeletedObjects = ( data, eventMap ) => {
    ...
    if( !_.isEmpty( deletedDepInfos ) ) {
        eventBus.publish( 'scheduleNavigationTree.dependenciesDeleted', { deletedDepInfos } );
    }
    ...
    if( selectedObjects && selectedObjects.length > 0 ) {
        ...
        if( delDepIndex > -1 ) {
            ...
            newSelection = locationContext.modelObject;
        }
        if( newSelection ) {
            selectionService.updateSelection( newSelection, locationContext.modelObject );
        }
    }
};
