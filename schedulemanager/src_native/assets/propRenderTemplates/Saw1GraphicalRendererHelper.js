...
export let renderPredecessors = function( vmo, containerElem ) {
    ...
    if( vmo && vmo.props && vmo.props.saw1Predecessors && vmo.props.saw1Predecessors.dbValues[0] !== null && ( taskDepDispValue !== vmo.props.saw1Predecessors.dbValues[0]) ){
        if( vmo.props.saw1Predecessors.displayValues[0] !== vmo.props.saw1Predecessors.prevDisplayValues[0] ){
            taskDepDispValue = scheduleNavigationTreeRowService.saveDependencyEdits( vmo, 'saw1Predecessors', taskDepDispValue, vmo.props.saw1Predecessors.dbValues[0] );
        }
    }
    let childElement = document.createElement( 'div' );
    childElement.className = 'aw-splm-tableCellText';
...
export let renderSuccessors = function( vmo, containerElem ) {
    ...
    if( vmo && vmo.props && vmo.props.saw1Successors && vmo.props.saw1Successors.dbValues[0] !== null && (taskDepDispValue !== vmo.props.saw1Successors.dbValues[0]) ){
        if( vmo.props.saw1Successors.displayValues[0] !== vmo.props.saw1Successors.prevDisplayValues[0]){
            taskDepDispValue = scheduleNavigationTreeRowService.saveDependencyEdits( vmo, 'saw1Successors', taskDepDispValue, vmo.props.saw1Successors.dbValues[0] );
        }
    }
    let childElement = document.createElement( 'div' );
    childElement.className = 'aw-splm-tableCellText';
