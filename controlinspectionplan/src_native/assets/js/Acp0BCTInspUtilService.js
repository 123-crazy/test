...
class Acp0BCTInspUtilService {
    ...
    get preferencesPromise() {
      const promise = preferenceService.getMultiStringValues(PREFERENCE_NAMES).then(parsePreferenceResponse).catch(error => {
        throw new Error( _dataI18n.Acp0PrefNotLoaded + error );
      });
     ...
     */
    async getPartAttachmentProject(selectedUid, dataI18n) {
      _dataI18n = dataI18n;
      ...
      if (attachments.length === 0) {
        throw new Error(_dataI18n.Acp0NoAttachmentMatchingPref);
      }
    ...
    async loadProjectFromAttachment(attachments) {
      ...
      appCtxService.ctx.fileTypeOfProject = relation.fileType.toUpperCase();
  ...
  try{
    ...
    switch ( error.message ) {
      case 'IPXML with multiple JT models are not supported':
          appCtxService.ctx.renderingErrorMessage = _dataI18n.Z_3DRenderingErrorIpxmlWithMultiJTModel;
          break;
      case 'IPXML does not contain JT model-related data':
          appCtxService.ctx.renderingErrorMessage = _dataI18n.Z_3DRenderingErrorInvalidIpxml;
          break;
      case 'IPXML requires a JT model':
          appCtxService.ctx.renderingErrorMessage = _dataI18n.Z_3DRenderingErrorAttachJT;
          break;
...
function parsePreferenceResponse(response) {
    ...
    if (missing.length > 0) {
      throw new Error( _dataI18n.Acp0NotGetReqPreference + missing.join(', ') );
    }
 ...
 *
 * @memberof NgServices
 * @member Acp0BCTInspUtilService
 */
app.factory( 'Acp0BCTInspUtilService', () => exports );
