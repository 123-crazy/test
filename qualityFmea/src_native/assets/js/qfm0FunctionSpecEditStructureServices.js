 ...
 *
 * @module js/qfm0FunctionSpecEditStructureServices
 */
...
import qfm0FunctionSpecTreeTableService from 'js/qfm0FunctionSpecTreeTableService';
...
export let addNewElement = function( addElementInput ) {
    var updatedParentElement = addElementInput && addElementInput.props.qfm0ParentTag.dbValues[ 0 ] ? addElementInput.props.qfm0ParentTag.dbValues[ 0 ] : null;
    if( updatedParentElement === null ) {
        appCtxService.ctx.functionSpecManagerContext.parentNode = addElementInput;
    } else if( appCtxService.ctx.locationContext[ 'ActiveWorkspace:SubLocation' ] === 'qualityfunctionspecificationmanager' && updatedParentElement && updatedParentElement === appCtxService.ctx
        .locationContext.modelObject.uid ) {
        appCtxService.ctx.functionSpecManagerContext.parentNode = addElementInput;
    }
    _insertAddedElementIntoVMOForSelectedParent( appCtxService.ctx.treeVMO.viewModelCollection, appCtxService.ctx.treeVMO.topTreeNode, updatedParentElement, addElementInput )
        .then(
            function() {
                ...
                eventBus.publish( 'qfm0FunctionSpecSelectionUpdateEvent', eventData );
            }
        );
};
...
function _insertAddedElementIntoVMOForSelectedParent( viewModelCollection, topTreeNode, inputParentElement,
    ...
    if( parentIdx > -1 ) {
        ...
        // Add root function specification.
...
function _insertSingleAddedElementIntoParentVMOAndViewModelCollection( viewModelCollection,
    ...
    if( childOccRespIdx !== 0 ) {
        ...
        var childVMO = qfm0FunctionSpecTreeTableService.createVmNodeUsingNewObjectInfo( childOccurrence, childlevelIndex, childIdx, true );
...
function _updateParentNodeState( deferred, viewModelCollection, parentUid, parentIdx, newElements, topTreeNode ) {
    ...
    if( parentIdx > -1 ) {
        ...
        soaInput2.searchInput.searchCriteria.catalogueObjectType = 'Qfm0FunctionEleSpec';
...
app.factory( 'qfm0FunctionSpecEditStructureServices', () => exports );
