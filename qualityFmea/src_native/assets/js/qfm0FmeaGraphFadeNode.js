 ...
 */
/**
 * This module defines qfm0FmeaGraphFadeNode
 *
 * @module js/qfm0FmeaGraphFadeNode
 */
import qfm0GraphService from 'js/qfm0FmeaGraphService';
'use strict';
 ...
 * @param {int} length of the animation time (ms).
 * @param {function} draw the function to draw the animation.
 * @param {function} timing the function to calculate animation progress. Gets a time fraction from 0 to 1, returns the
    ...
    if( node.fadeAnimationId && node.cancelFadeAnimation ) {
        ...
        startFadeNodeAnimation( node, 1500, function finishCallback( bWasCancelled ) {
            if( !bWasCancelled ) {
                ...
                qfm0GraphService.resolveConnectedGraph( graphModel );
            }
        } );
    }
};
