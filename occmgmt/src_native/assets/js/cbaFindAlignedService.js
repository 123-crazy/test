...
*/
export let getAlignedObjectsInfo = function( syncObject ) {
    ...
    if ( alignedObjectInfo.alignedObjects.length > 0 ) {
        for ( let index = 0; index < alignedObjectInfo.alignedObjects.length; index++ ) {
            ...
            alignedObjectsList.push( { uid:updatedUid } );
        }
        alignedObjectInfo.alignedObjects = alignedObjectsList;
        alignedObjectInfo.firstObjectToFind = alignedObjectInfo.alignedObjects[0];
...
app.factory( 'cbaFindAlignedService', () => exports );
