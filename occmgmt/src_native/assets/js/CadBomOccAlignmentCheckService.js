 ...
 */
 let _isExpansionCase = function( subPanelContext ) {
    ...
    let expandedNodes = _.get( appCtxSvc.ctx, `${activeContext}.getOccInput.inputData.requestPref.expandedNodes` );
    let expandedNodesSize = !_.isUndefined( expandedNodes ) ? expandedNodes.length : 0;
    let startFreshNavigation = _.get( appCtxSvc.ctx, `${activeContext}.getOccInput.inputData.requestPref.startFreshNavigation[0]` );

    let focussedOccUid = _.get( appCtxSvc.ctx, `${activeContext}.getOccInput.inputData.focusOccurrenceInput.element.uid` );
    let isAutoExpantion = focussedOccUid && !_.isEmpty( focussedOccUid ) && 'AAAAAAAAAAAAAA' !== focussedOccUid;
    return ( _.isEqual( expandedNodesSize, 0 ) || isAutoExpantion ) && _.isEqual( startFreshNavigation, 'false' );
};
