...
export let initRelationContext = function( ctx, data ) {
    ...
    delete ctx.relationSelection;
    delete ctx.referenceTopicType;
    delete ctx.ctm0KeyName;
    ctx.referenceListValues = [];
};
 ...
 */
export let updateRelationSelectionEnablement = function( ctx ) {
    if( ctx.relationSelection ) {
        if ( ctx.ctm1.hasTopicTopicR === "false" ){
            ctx.relationSelection.dbValue = "DC_ComposableReferenceR";
            ctx.relationSelection.isEnabled = false;
        }
        else if ( ctx.relationSelection.isEnabled === false ) {
            ctx.relationSelection.isEnabled = true;
        }
    }
...
export let createRefTopicTypeRelation = function( referenceTopicTypeeUid, keyName, data ) {
    occmgmtBackingObjProviderSvc.getBackingObjects(data.addElementResponse.selectedNewElementInfo.newElements).then(function (bomLines) {
        let inputData =  {
            "input" :[ {
