 ...
 */
 export let createLink = function( type, source, target ) {
    var sourceObject = _cdm.getObject( source );
    var targetObject = _cdm.getObject( target );
    ...
    if( sourceObject && targetObject ) {
        ...
        if( _cmm.isInstanceOf( 'Prg0AbsEvent', sourceObjectType ) &&
            _cmm.isInstanceOf( 'Prg0AbsEvent', targetObjectType ) ) {
            ...
            if( dateTimeService.compare( targetObjDate, sourceObjDate ) >= 0 ) {
                var info = {
                    predTask: sourceObject,
                    succTask: targetObject
                };
                eventBus.publish( 'linkCreatedViaDrag', info );
            } else {
        ...
        let sourceObj = _cdm.getObject( source );
        let targetObj = _cdm.getObject( target );

        let milestoneDepCreationErrorMessage = localTextBundle.Pgp0EventMilestoneDepCreateErrorMsg;
        let finalMessage = messagingService.applyMessageParams( milestoneDepCreationErrorMessage, [ '{{sourceEvent}}', '{{targetEvent}}' ], {
            sourceEvent: sourceObj.props.object_name.dbValues[ 0 ],
            targetEvent: targetObj.props.object_name.dbValues[ 0 ]
        } );
        messagingService.showError( finalMessage );
    }
};
 ...
 *
 */
export let formatNewEventDate = function( data, eventObj ) {
    var dateValue;
    //When reading the new date from Shift Event panel
    if( data.Pgp0NewPlannedDate && data.Pgp0NewPlannedDate.dbValue ) {
        if( eventObj.props.prg0PlannedDate ) {
            let plannedDate = new Date( eventObj.props.prg0PlannedDate.dbValues[ 0 ] );
            let newPlannedDate = new Date( data.Pgp0NewPlannedDate.displayValues[ 0 ] );
    ...
    else if( data.eventData.plannedDate ) {
        dateValue = dateTimeService.formatUTC( data.eventData.plannedDate );
    }
    //When event is dragged on Timeline while no secondary events are present
    else if( data.eventData.updateTaskInfo.plannedDate ) {
        dateValue = dateTimeService.formatUTC( data.eventData.updateTaskInfo.plannedDate );
    }
    return dateValue;
};
 ...
 */
export let setUpdateSecondaryEvents = function( data ) {
    //If event has secondary events then returns the value set in the checkbox
    if( data.Pgp0ShiftSecondary ) {
        return data.Pgp0ShiftSecondary.dbValue;
    }
 ...
 * @param {Object} newSelection - selected dependency link id
 */
export let selectLink = function( newSelection ) {
    let link = getEventDependency( newSelection );
    if( link ) {
        selectionSvc.updateSelection( link, ctxService.ctx.locationContext.modelObject );
    }
    var oldSelection = timelineManager.getSelectedLink();
    timelineManager.setSelectedLink( newSelection );
    if( oldSelection !== newSelection ) {
        prgtimelineUtils.selectLink( oldSelection );
        prgtimelineUtils.selectLink( newSelection );
    }
    eventBus.publish( 'updateSWAForEvent', link );
};
