...
/**
 * Populate group user LOV values.
 *
 * @param {Object} data Data view model object
 */
...
/**
 * Sets the selected template on the view model object
 *
 * @param {Object} data Data view model object
 */
export let setSelectedTemplate = function( data ) {
    if( data.dataProviders.planTemplateSearchProvider ) {
        let curTemplate = data.dataProviders.planTemplateSearchProvider.selectionModel.getSelection();
        if( curTemplate ) {
            data.selectedTemplate = curTemplate[ 0 ];
            appCtxSvc.ctx.programPlanningContext.selectedTemplate = curTemplate[ 0 ];
        }

        let destPanelId = "ShowCreateTypes";
        // let destPanelId = "Pgp0CreateFromTemplate";
        let context = {
            destPanelId: destPanelId,
            "recreatePanel": false
        };
        ...
        eventBus.publish( "Pgp0CreateFromTemplate.refreshTab", refreshContext );
...
/**
 * Adds the selected template from the view model object to the data provider
 *
 * @param {Object} data Data view model object
 */
export let getSelectedTemplate = function( data ) {
    if( data.selectedTemplate && data.dataProviders.selectedTemplateProvider.viewModelCollection.loadedVMObjects.length === 0 ) {
        data.dataProviders.selectedTemplateProvider.viewModelCollection.loadedVMObjects.push( data.selectedTemplate );
    }
};
...
/**
 * Adds the selected event from the view model object to the data provider
 *
 * @param {Object} data Data view model object
 */
 export let addSelectedEvent = function( data ) {
    if( data.selectedEvent && data.dataProviders.selectedEventProvider.viewModelCollection.loadedVMObjects.length === 0 ) {
        data.dataProviders.selectedEventProvider.viewModelCollection.loadedVMObjects.push( data.selectedEvent );
    }
};

/**
 * Removes the selected event from the command context
 *
 * @param {Object} data Data view model object
 */
 export let removeEventFromProvider = function( data ) {
    data.dataProviders.selectedEventProvider.viewModelCollection.loadedVMObjects.pop();
    data.dataProviders.selectedEventProvider.selectedObjects.pop();
    delete appCtxSvc.ctx.programPlanningContext.selectedEvent;
};
...
/**
 * Prepares the SaveAs input
 *
 * @param {Object} data Data view model object
 * @param {Object} context  Context object
 */
 export let getSaveAsInputForEvent = function( data, context ) {
    updateContextForEvent( data, context );
    let saveAsInput = saveAsService.getSaveAsInput( data, context );

    saveAsInput[ 0 ].saveAsInput.tagProps = {};
    if( appCtxSvc.ctx.xrtPageContext.primaryXrtPageID ==="tc_xrt_Timeline" ){
        saveAsInput[ 0 ].saveAsInput.tagProps.prg0TargetParentPlan = context.selected;
    } else {
        saveAsInput[ 0 ].saveAsInput.tagProps.prg0TargetParentPlan = appCtxSvc.ctx.xrtSummaryContextObject;
    }
 ...
 * @param {Object} context  Context object
 */
export let getSaveAsInput = function( data, context ) {
    ...
    if( data.isTemplate.dbValues[ 0 ] === true ) {
        if( !saveAsInput[ 0 ].saveAsInput.boolProps ) {
            saveAsInput[ 0 ].saveAsInput.boolProps = {};
        }
        saveAsInput[ 0 ].saveAsInput.boolProps.prg0IsTemplate = true;
    }
...
/**
 * Retrieves the newly created object from the response
 *
 * @param {Object} response response from the SaveAs operation
 */
export let getNewCreatedObject = function( response ) {
    return saveAsService.getNewCreatedObject( response );
};
 ...
 * @param {Object} context  Context object
 */
export let refreshFromTemplateTab = function( data, ctx ) {
    ctx.programPlanningContext.selectedTemplate = data.selectedTemplate;
};
 ...
 * @param {Object} context  Context object
 */
export let destroy = function( data, ctx ) {
    if( ctx.programPlanningContext.addPlanParent ){
        delete ctx.programPlanningContext.addPlanParent;
    }
    if( ctx.programPlanningContext.selectedTemplate ){
        if( ctx.SelectedObjects && ctx.SelectedObjects[0] === ctx.programPlanningContext.selectedTemplate ){
            ctx.SelectedObjects.splice(0,1);
        }
        delete ctx.programPlanningContext.selectedTemplate;
    }
};
...
export default exports = {
    ...
    removeEventFromProvider,
    getSaveAsInput,
    getNewCreatedObject,
    refreshFromTemplateTab,
    modifySaveAsForm,
