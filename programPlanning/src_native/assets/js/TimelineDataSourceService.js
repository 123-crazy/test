 ...
 * @param {data} data - Contains the data providers result
 */
export let buildResultString = function( data ) {
    var resultStr = '(' + data.totalFound + ' ' + data.i18n.pgp0EventResults + ')';
    data.resultString.dbValue = resultStr;
};
...
export let select = function( id ) {
    ...
    eventBus.publish( 'updateSWAForEvent', task );
    let locationContext = ctxService.getCtx( 'locationContext' );
    selectionSvc.updateSelection( task, locationContext.modelObject );
};
...
export let onObjectsDeleted = function( eventMap ) {
    ...
    if( deletedTimelineUids.length > 0 ) {
        if( deletedTimelineUids.includes( ctxService.ctx.timelineContext.selected.uid ) ) {
            eventBus.publish( 'updateSWAForEvent', ctxService.ctx.locationContext.modelObject );
        }
 ...
 */
export let onObjectsUpdated = function( eventMap, data ) {
    if( eventMap ) {
