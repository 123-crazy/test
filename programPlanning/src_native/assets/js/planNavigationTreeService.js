...
export let loadTreeTableData = function( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, isInitialLoad ) {
    ...
    return AwPromiseService.instance.resolve( _buildTreeTableStructure( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, isInitialLoad, declViewModel ) );
};
 ...
 */
 export let setColumnFilterTimeline = function( context ) {
    let columnFilters = [];
    if( context && context.columnProvider ) {
       columnFilters = context.columnProvider.columnFilters;
       if( columnFilters && columnFilters.length > 0 ) {
            appCtxSvc.updateCtx( 'isColumnFilteringApplied', true );
            appCtxSvc.ctx.showHideEventDependencyFlag = false;
            planNavTimelineSync.clearAllTasks();
            let rootTreeNode = planNavTreeUtils.getRootTreeNode();
            uiTimelineUtils.parseGanttData( rootTreeNode, null );
       }else {
            appCtxSvc.updateCtx( 'isColumnFilteringApplied', false );
            planNavTimelineSync.clearAllTasks();
       }
    }
};
 ...
 *
 */
var _buildTreeTableStructure = function( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, isInitialLoad, declViewModel ) {
 ...
 */
export let getListOfPlanObj = function( data ) {
    let planUidString = '';
    if( appCtxSvc.getCtx( 'isColumnFilteringApplied' ) ) {
        let resultTree = data.treeLoadResult;
        for( let  idx = 0; idx < resultTree.length; idx++ ) {
            planUidString += resultTree[idx].uid + ',';
        }
 ...
 * @param {*} data
 */
export let processEvents = function( soaResponse, data ) {
    ...
    if( appCtxSvc.ctx.showTimeline && appCtxSvc.ctx.planNavigationCtx.isTimelineInitialized ) {
        planNavTimelineSync.addEventObjectsInTimeline( eventObjs, data );
        ...
        if( appCtxSvc.ctx.showHideEventDependencyFlag && eventObjs.length > 0 ) {
            eventBus.publish( 'processedEventsInfo', eventObjs );
        }
        let eventData = {};
 ...
 */
export let updateCtxWithCommandConditionData = () => {
    let planNavigationCtx = appCtxSvc.getCtx( 'planNavigationCtx' );
    if( !planNavigationCtx ) {
        planNavigationCtx = {};
    }

    let viewModel = viewModelService.getViewModelUsingElement( planNavTreeUtils.getPlanNavigationTreeTableElement() );
    if( viewModel ) {
        let vmNodes = viewModel.dataProviders.planNavigationTreeDataProvider.getSelectedObjects();

        // Update the parent of selected node in the ctx.
        if( vmNodes && vmNodes.length === 1 && !_.isEmpty( vmNodes[0].parentNodeUid ) ) {
            let vmCollection = viewModel.dataProviders.planNavigationTreeDataProvider.viewModelCollection;
...
app.factory( 'planNavigationTreeService', () => exports );
