 ...
 */
 function getChangeSummaryDataSOA( treeLoadInput, dataProvider ) {
    ...
    var inputData = cmUtils.getChangeSummaryInputData( appCtxSvc.ctx, treeLoadInput, dataProvider );
...
function createTreeNode( vmo, index, treeLoadInput, firstColumnName ) {
    ...
    //set action name
    vmNode.actioName =  vmo.props.action.dbValues[0];
...
var _generateOverrideContextIcon = function( vmo, cellContent, imgTitle ) {
    let imagePath = app.getBaseUrlPath() + '/image/indicatorOverridden16.svg';
...
var _addExtendedTooltip = function( cellContent, column, vmo, tableElem ) {
    ...
    _populateOverrideContextIcon( vmo, column, compiledCellParent, tableElem );
...
export let resetColumnConfig = function( dataProvider ) {
    ...
    // For reloading Change Summary Table after reseting column configuration, we need to set startIndexForNextPage to 0.
