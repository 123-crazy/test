...
( function() {
    ...
    CKEDITOR.cleanWord = function( mswordHtml, editor ) {
        ...
        function oleShapeTagging( element ) {
            if ( element.attributes[ 'o:ole' ] !== undefined ) {
                oleShapesIds.push( element.attributes.id );
            }
        }
        ...
        var filterDefinition = {
            ...
            elements: {
                ...
                'img': function( element ) {
                    ...
                    // check if image is for ole
                    // if yes, remove it, as copying ole is not supported yet
                    if(imgShapesIds && imgShapesIds.length > 0) {
                        var isShapeFromOLE = CKEDITOR.tools.array.every( imgShapesIds, function( shapeId ) {
                            return oleShapesIds.indexOf( shapeId ) > -1;
                        } );
                    }
        ...
        extractFromRtf: function( rtfContent ) {
            ...
            for ( var i = 0; i < wholeImages.length; i++ ) {
                if ( rePictureHeader.test( wholeImages[ i ] ) ) {
                    ...
                    ret.push( {
                        hex: wholeImages[ i ].replace( rePictureHeader, '' ).replace( /[^\da-fA-F]/g, '' ),
                        type: imageType
                    } );
                }
            }
