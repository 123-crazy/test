...
const sortedAscendingIcon = iconService.getIcon( 'miscSortedAscending' );
...
export let set_visualization_size = function() {
    ...
    if( commandBars.length > 0 ) {
        let index = 0;
        if( appCtxSvc.ctx.xrtPageContext.primaryXrtPageID !== 'tc_xrt_Traceability' && commandBars.length > 1 ) {
            index = 1;
        }
        commandBarWidth = document.getElementsByClassName( 'aw-commandBar-horizontal' )[ index ];
    ...
    var tracelinkTable = document.getElementsByTagName( 'aw-splm-table' );
...
export let deteleTracelinkFromMap = function( linkInfo, elementsInDeleteTracelink ) {
    if( linkInfo ) {
        ...
        //Delete or update tracelink direct from map
        if( linkInfo.complyingLinksInfo.length > 0 && linkInfo.definingLinksInfo.length > 0 ) {
            linkInfo.tracelinkDirection = 'Both';
        }else if( linkInfo.complyingLinksInfo.length === 0 && linkInfo.definingLinksInfo.length === 0 ) {
            linkInfo.tracelinkDirection = '';
        }else if( linkInfo.complyingLinksInfo.length > 0 ) {
            linkInfo.tracelinkDirection = 'Complying';
        }else if( linkInfo.definingLinksInfo.length > 0 ) {
            linkInfo.tracelinkDirection = 'Defining';
        }
    }
    return linkInfo;
};
    ...
    if( treeMode && data.level && data.isHidden ) {
        for( var i = 0; i < data.level; i++ ) {
            cell.append( 'line' ).attr( 'x1', 12 * i + 5 ).attr( 'y1', -30 )
                .attr( 'x2', 12 * i + 5 ).attr( 'y2', 5 ).style( 'stroke-dasharray', 1 ).style( 'stroke-width', 1 );
        }
...
var _handleShiftSelection = function( data, isRow, clear, id, isMultipleSelection, isShiftSelection ) {
    ...
    if( load ) {
     ...
     // selectedOccurrence ctx required for expand all command
     var selectedOcc = [];
     for( let index = 0; index < selectionUids.length; index++ ) {
 ...
 * @return {Object} objects - updated data elements
 */
var _checkMultiselection = function( isRow, isHeader ) {
 ...
 */
var _handleObjectSelection = function( data, isRow, clear, unselect, id, isMultipleSelection, isShiftSelection, obj ) {
 ...
 */
// eslint-disable-next-line complexity
export let handleRowSelection = function( data, isRow, isHeader, clear ) {
    var isMultipleSelection = false;
    var unselect = false;
    var id;
    var obj;
    ...
    if( clear ) { //when the selected cell is unselected
        ...
        var output = _checkMultiselection( isRow, isHeader );
...
export let handleCellSelection = function( element, data ) {
    ...
    if( !isMultipleSelection ) {
        exports.handleRowSelection( data, true, false, clear );
...
export let render_popup = function( d, hiddenObjectsPopupData ) {
    ...
    popupService.show( popupData ).then( function( popupRef ) {
        ...
        var eventData = d;
        eventBus.publish( hiddenObjectsPopupData.declView + '.reveal', eventData );
    } );
};
