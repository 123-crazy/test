...
export function createListItems( Container, editor ) {
    ...
    for ( var wdgt in reqWidgets ) {
        ...
        var text;
        var uid;
        if( widget.tagName === 'LOADING' ) {
            text = widget.getElementsByTagName( 'h3' )[0].innerText;
            uid = widget.getAttribute( 'id' );
        } else {
            var reqElement = widget.querySelector( 'div.aw-requirement-header h3' );
            if ( reqElement ) {
                var reqId = widget.querySelector( 'span.aw-requirement-headerNonEditable' ).innerText;
                var reqPropElement = widget.querySelector( 'span.aw-requirement-properties' );
                text = reqId;
                if( reqPropElement ) {
                    text += reqPropElement.innerText;
                }
                uid = widget.getAttribute( 'revisionid' );
            }
        }
