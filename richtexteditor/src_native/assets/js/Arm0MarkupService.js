...
export let setContext = function() {
    ...
    var swaCtx = exports.getContext( 'xrtPageContext' );
    if ( swaCtx.secondaryXrtPageID !== 'tc_xrt_summary_table' ) {
...
export let selectMarkup = function( markup, toScroll ) {
    ...
    if ( markup.visible && markup !== markupCtx.currentSelection ) {
        if ( markupCtx.showPanel ) {
            ...
            if ( toScroll && swaCtx.secondaryXrtPageID !== 'tc_xrt_summary_table' || widePanelCtx ) {
                scrollIntoView( markup );
            }
        } else if ( swaCtx.secondaryXrtPageID === 'tc_xrt_summary_table' && !widePanelCtx ) {
...
export let saveMarkupEdit = function( data, isCk5 ) {
    if ( data.markup ) {
        ...
        var widePanelCtx = appCtxSvc.getCtx( 'Arm0SingleRequirementWidePanelEditorActive' );
        if ( !isCk5 && data.markup.editMode !== 'reply' && swaCtx.secondaryXrtPageID !== 'tc_xrt_summary_table' || widePanelCtx ) {
 ...
 */
export let markupSelectedInPanel = function( eventData ) {
    var swaCtx = appCtxSvc.getCtx( 'xrtPageContext' );
    var widePanelCtx = appCtxSvc.getCtx( 'Arm0SingleRequirementWidePanelEditorActive' );
    if ( swaCtx.secondaryXrtPageID === 'tc_xrt_summary_table' && !widePanelCtx ) {
        exports.markupSelected( eventData );
    } else {
