...
export default class CommentsAdapter extends CKEDITOR5.Plugin {
    constructor( editor ) {
        super( editor );
    }
...
class MouseOverObserver extends CKEDITOR5.DomEventObserver {
    constructor( view ) {
        super( view );
...
class MouseLeaveObserver extends CKEDITOR5.DomEventObserver {
    constructor( view ) {
        super( view );
...
function _subscribeDOMMouseEvents( editor, viewDocument ) {
    ...
    editor.listenTo( viewDocument, 'mouseover', ( evt, data ) => {
        if ( data.domTarget && data.domTarget.classList && data.domTarget.classList.contains( 'ck-comment-marker' )
            && data.domTarget.dataset && data.domTarget.dataset.comment ) {
            var threadId = data.domTarget.dataset.comment;
            if( threadId ) {
