...
export let initContent = function( data, ctx ) {
    ...
    if ( data.content ) {
        ...
        if ( transientFileReadTickets ) {
            ...
            data.htmlString = data.htmlString.replace( regexStart, '<suggestion-start name=$1></suggestion-start>' );

            const regexEnd = /<*suggestion-end name+\s*=\s*("([^"]*)")*\s*\/>/g;
            data.htmlString = data.htmlString.replace( regexEnd, '<suggestion-end name=$1></suggestion-end>' );
