 ...
 */
  ...
  */
 export let showDeleteTracelinkTMWarning = function( i18n, vmo ) {
     ...
     var msg = i18n.deleteTracelinkConfirmation.replace( '{0}', name );
     var buttons = [ {
         ...
         onClick: function( $noty ) {
             $noty.close();
             eventBus.publish( 'Arm0TraceabilityMatrix.deleteExistingTracelinkInTM' );
         }
     } ];
  ...
  * @param {Object} i18n - The view model data
  */
 export let showDeleteAllTracelinkTMWarning = function( i18n ) {
     var selectedObjects = _data.dataProviders.traceMatrixCellPropDataProvider.getViewModelCollection().loadedVMObjects;
     var name = '';
     if( selectedObjects.length > 0 ) { name = selectedObjects[ 0 ].props.object_complying.displayValue; }

     var msg = i18n.deleteAllTracelinkConfirmation.replace( '{0}', name );
     var buttons = [ {
         addClass: 'btn btn-notify',
         text: i18n.cancel,
         onClick: function( $noty ) {
  ...
  */
 export let getInputDeleteTraceLinkInTM = function( data, vmo ) {
     var selectedObjects;
     if( vmo ) {
         selectedObjects = vmo.getSelectedObjects();
     }
 ...
 let calculateFilterOptions = function( data, response ) {
     ...
     if( response.filterOutput && data.filterOutput.searchFilterMap ) {
 ...
 export let populateFilterInformation = function( data, ctx ) {
     var activeFilterView = ctx.MatrixContext.activeFilterView;
     ...
     appCtxService.registerPartialCtx( 'MatrixContext.tracelinkFilterOutput', ctx.MatrixContext.typeFilter );
 };
 ...
 export let processDataFromServer = function( data, ctx ) {
     if( !ctx.MatrixContext.matrixObj && ctx.locationContext.modelObject.type === 'Awp0TraceabilityMatrix' ) {
         ctx.MatrixContext.matrixObj = ctx.locationContext.modelObject;
     }else if( !ctx.MatrixContext.matrixObj && ctx.selected.type === 'Awp0TraceabilityMatrix' ) {
         ctx.MatrixContext.matrixObj = ctx.selected;
     }
         ...
         */
         ...
         if( !ctx.tlmTreeMode ) {
             ...
             if( ctx.MatrixContext.parentCols && !parentColUid ) {
                 ...
                 if( !( noSortCol || noSortRow || ctx.MatrixContext.sortRow || ctx.MatrixContext.sortCol ) ) {
                     var lastRowObj;
                     var allExpandedRows = [];
             ...
             // Calculating expanded rows/cols not required for in case of sort
             if( ctx.MatrixContext.parentRows && !parentRowUid && !( noSortCol || noSortRow || ctx.MatrixContext.sortRow || ctx.MatrixContext.sortCol ) ) {
                 // check response cols for expanded parents
                 var allExpandedCols = [];
                 var lastColObj;
 ...
 export let getSettingProperties = function( data ) {
     ...
     if( data.preferences.REQ_Matrix_Settings_Properties && data.preferences.REQ_Matrix_Settings_Properties.length > 0 ) {
         data.preferences.REQ_Matrix_Settings_Properties.forEach( objProp => {
 ...
 let addInTracelinkIDVsPersistentObjMap = function( linkInfo ) {
     ...
     if( linkInfo.secondary && linkInfo.secondary.modelType && linkInfo.secondary.modelType.typeHierarchyArray && linkInfo.secondary.modelType.typeHierarchyArray.indexOf( 'Awb0ConditionalElement' ) >= 0 ) {
 ...
 var _updateParentLinkOnTracelinkCreate = function( data, primObj, secObj, linkInfo, deepCopiedLinkInfo ) {
     if ( data.matrixType === 'Full-Rollup Matrix' ) {
         ...
         // if( MatrixContext.parentCols || MatrixContext.parentCols ) {
         //     var parentsRowobject = MatrixContext.parentRows;
         //     var parentsColobject = MatrixContext.parentCols;
         // }else{
 ...
 export let generateTraceabilityMatrix = function( replaceType, selectedObject, ctx ) {
     if( selectedObject ) {
         ...
         if( ctx.MatrixContext ) {
             if( ctx.MatrixContext.peakTargetInfo ) {
                 targetInfo = ctx.MatrixContext.peakTargetInfo.occurrence;
                 if( targetInfo ) {
                     arrModelObjs.push( {
                         uid: targetInfo.uid
                     } );
             ...
             if( ctx.MatrixContext.peakSrcInfo ) {
                 srcInfo = ctx.MatrixContext.peakSrcInfo.occurrence;
                 if( srcInfo ) {
                     arrModelObjs.push( {
                         uid: srcInfo.uid
                     } );
         ...
         requirementsUtils.loadModelObjects( arrModelObjs, cellProp ).then( function() {
             ...
             _data.replacePayload = {
                 ...
                 srcObjectInfo: { occurrence: ctx.sourceObjects[ 0 ] },

                 targetObjectInfo: { occurrence: ctx.targetObjects[ 0 ] },
                 srcContextInfo: ctx.MatrixContext.sourcePCI,
                 targetContextInfo: ctx.MatrixContext.targetPCI,
                 matrixType: _data.matrixType
             };

             getTraceabilityMatrix2( _data, ctx, _data.replacePayload );
         } );
     }
 };
     ...
     if( fmsTicket ) {
         $.get( exports.getFileUrl( fmsTicket ), function( data1 ) {
             ...
             if( !jsonObject.isSaved && jsonObject.matrixType !== 'Full-Rollup Matrix' ) {
                 ...
                 getTraceabilityMatrix2( data, ctx, jsonObject );
                 return;
             }
  ...
  */
 var getTraceMatrixInputQuick = function( matrix_startup_props, ctx ) {
  ...
  * @return {Array} input data
  */
 export let getTraceMatrixFilterInput_old = function( data, ctx ) {
     var isAsyncCall = false;
     var showChildrenCount = false;
     var itemsPerPage = -1;
     var srcPCI = {};
     ...
     var rowFilter = {};
     var colFilter = {};
     var matrixContext = ctx.MatrixContext;

     if( ctx.matrixType === 'Dynamic Matrix' ) {
     ...
     if( ctx.matrixType === 'Quick Matrix' ) {
         ...
         if( matrixContext.colFilter && matrixContext.colFilter.categoriesInfo ) {
             var colFiltercategoriesInfo = matrixContext.colFilter.categoriesInfo;
             var colFilterCategories = colFiltercategoriesInfo.filterCategories;
             var colFilterMap = colFiltercategoriesInfo.filterMap;
             colFilter = {
                 fetchUpdatedFilters: false,
                 recipe: [],
                 searchFilterCategories: colFilterCategories,
                 searchFilterFieldSortType: 'Priority',
                 searchFilterMap: colFilterMap,
                 searchSortCriteria: []
             };
         }
 ...
 var getTraceabilityMatrix2 = function( data, ctx, payLoad ) {
     if( payLoad ) {
         ...
         if( data.matrixType === 'Dynamic Matrix' ) {
             var soaInput = {
                 inputData: getTraceMatrixInputDynamic( payLoad, ctx )
             };
             if( data.replacePayload && data.replacePayload.info === 'req_replace_matrix_payload2' ) {
             ...
             var soaInput = {
                 inputData: getTraceMatrixInputQuick( payLoad, ctx )
             };
             if( data.replacePayload && data.replacePayload.info === 'req_replace_matrix_payload2' ) {
                 soaInput.inputData.options = [ 'GENERATE_MATRIX' ];
             }
 ...
 eventBus.subscribe( 'Arm0FilterTM.contentUnloaded', function() {
 ...
 export let resetMatrixCommandClick = function( ctx ) {
     ...
     var matrixObj = ctx.MatrixContext.matrixObj;
     resetMatrixContext( ctx );
     resetTreeStructure( ctx );
     requirementsUtils.getTraceabilityMatrixFMSTicket( matrixObj ).then( function( fmsTicket ) {
         loadTraceabilityMatrix( _data, ctx, fmsTicket );
     } );
 };
 ...
 export let getChildMatrixInput = function( ctx, data ) {
     ...
     var rowFilter = {};
     var colFilter = {};
     // if( ctx.MatrixContext.rowFilter && ctx.MatrixContext.rowFilter.categoriesInfo ) {
     //     var rowFiltercategoriesInfo = ctx.MatrixContext.rowFilter.categoriesInfo;
     //     var rowFilterCategories = rowFiltercategoriesInfo.filterCategories;
     //     var rowFilterMap = rowFiltercategoriesInfo.filterMap;
     //     rowFilter = {
     //         fetchUpdatedFilters: false,
     ...
     // if( ctx.MatrixContext.rowFilter && ctx.MatrixContext.rowFilter.filter ) {
     //     rowFilter = ctx.MatrixContext.rowFilter.filter;
     // }
     // if( ctx.MatrixContext.colFilter && ctx.MatrixContext.colFilter.filter ) {
     //     colFilter = ctx.MatrixContext.colFilter.filter;
     // }

     if( ctx.MatrixContext.peakTargetInfo && ctx.MatrixContext.peakTargetInfo.occurrence &&
         ctx.MatrixContext.peakTargetInfo.occurrence.uid && ctx.MatrixContext.peakTargetInfo.occurrence.uid === ctx.MatrixContext.peakSrcInfo.occurrence.uid ) {
         colFilter = rowFilter;
     }

     var option = [];
     if( ctx.MatrixContext.sourceFilterReceipeCount > 0 || ctx.MatrixContext.targetFilterReceipeCount > 0 ) {
     ...
     var inputData = {
         ...
         relationTypes: ctx.MatrixContext.typeFilter && ctx.MatrixContext.typeFilter.filter ? ctx.MatrixContext.typeFilter.filter : [ 'ALL' ],
 ...
 export let collapseNode = function( data, ctx ) {
     ...
     if( rowUid && ctx.MatrixContext.expandedRows.indexOf( rowUid ) !== -1 ) {
         ...
         ctx.MatrixContext.expandedRows = ctx.MatrixContext.expandedRows.filter( uid => !collapseRows.includes( uid ) );
     }
     ...
     if( colUid && ctx.MatrixContext.expandedCols.indexOf( colUid ) !== -1 ) {
         ...
         ctx.MatrixContext.expandedCols = ctx.MatrixContext.expandedCols.filter( uid => !collapseCols.includes( uid ) );
     }
     processData( data, ctx );
 ...
 export let updateMatrixAsSaved = function( ctx, data ) {
     _data.isSaved = true;
     ctx.MatrixContext.isMatrixSaved = true;
 };
 ...
 export let getTraceMatrixInputForSave = function() {
     ...
     if( _data.matrixType === 'Full-Rollup Matrix' ) {
         tracelinkInput = exports.getTraceMatrixInput( ctx, _data, true );
         tracelinkInput.showChildTracelinks = true;
         tracelinkInput.isRunInBackground = true;
     } else {
         tracelinkInput = exports.getTraceMatrixInput( ctx, _data, true );
     }
     tracelinkInput.traceabilityMatrixObject = { uid: ctx.MatrixContext.matrixObj.uid };
     ...
     if( ctx.showTracelinkDirection === true || ctx.showTracelinkDirection === 'true' ) {
 ...
 var _makeDataCompliant = function( data, linkMap ) {
     ...
     transformedData.srcObjectInfo = {};
     transformedData.targetObjectInfo = {};
     if( data.srcObjectInfo && data.srcObjectInfo.displayName !== '' && data.srcObjectInfo.occurrence && data.srcObjectInfo.occurrence.uid ) {
         var srcOcc = cdm.getObject( data.srcObjectInfo.occurrence.uid );
         transformedData.srcObjectInfo = {
             sort: 0,
             name: data.srcObjectInfo.displayName,
             uid: data.srcObjectInfo.occurrence.uid,
             isParent: false,
             type: srcOcc && srcOcc.modelType ? srcOcc.modelType.displayName : '',
             level: 0,
             parentUid: null,
             isExpanded: false,
             isRow: true,
             isHidden: false,
             isPreviousObjectHidden: false,
             isTopRow: true
         };
     }
     if( data.targetObjectInfo && data.targetObjectInfo.displayName !== '' && data.targetObjectInfo.occurrence && data.targetObjectInfo.occurrence.uid ) {
         var targetOcc = cdm.getObject( data.targetObjectInfo.occurrence.uid );
         transformedData.targetObjectInfo = {
             sort: 0,
             name: data.targetObjectInfo.displayName,
             uid: data.targetObjectInfo.occurrence.uid,
             isParent: false,
             type: targetOcc && targetOcc.modelType ? targetOcc.modelType.displayName : '',
             level: 0,
             parentUid: null,
             isExpanded: false,
             isRow: true,
 ...
 export let hideRowOrColumn = function( data, ctx ) {
     ...
     if( selectedObjs.length > 0 ) {
         ...
         if( ctx.MatrixContext.isRowSelected ) {
 ...
 export let createPopupData = function( data, ctx ) {
     var hiddenObject = data.eventData;
     ...
     data.columnValues = hiddenObjects;
     data.columnValuesWithoutFilter = hiddenObjects;
 ...
 export let unhideSelected = function( column, data, ctx ) {
     ...
     if( column[ 0 ].hiddenObject.isRow ) {
         ...
         ctx.MatrixContext.parentRows = objects;
     } else {
         _data.colObjects = objects;
         ctx.MatrixContext.parentCols = objects;
     }
     processData( _data, ctx );
     popupService.hide();
     reqTraceabilityMatrixService._handleUnselection();
 ...
 export let updateSelectAllProp = function( data ) {
     var current = data.selectAllProp;
     current.propApi = {};
 ...
 export let createFacetProp = function( hiddenObject, element, data ) {
     var prop = {
         propertyDisplayName: element.displayName,
         type: 'BOOLEAN',
         isRequired: false,
         isEditable: true,
         isEnabled: true,
     ...
     if( data.selectAllProp ) {
         updateSelectAllProp( data );
     }

     prop.propApi.fireValueChangeEvent = function() {
         if( !prop.dbValue && data.selectAllProp.dbValue ) {
             data.selectAllProp.dbValue = false;
         } else if( prop.dbValue && !data.selectAllProp.dbValue ) {
             ...
             if( uncheckedCount === 0 ) {
                 data.selectAllProp.dbValue = true;
             }
         }
         filterFacetInputChanged( data );
     };
 ...
 export let filterFacetInputChanged = function( data ) {
     if( areAllFacetsUnchecked( data ) || data.columnValues.length === 0 ) {
         disableUnhiding();
     } else {
         enableUnhiding();
     }
 ...
 export var filterAction = function( data ) {
     ...
     filterFacetInputChanged( data );
     data.columnValues.noFacetResults = false;
     if( data.columnValues.length === 0 ) {
         data.columnValues.noFacetResults = true;
     }
 };
 /**************************************** Upto Here ********************************************************* */
 ...
 */
 ...
 const _handleNoSort = ( transformedData, data ) => {
     ...
     if( noSortRow ) {
         ...
         appCtxService.ctx.MatrixContext.parentRows = data.rowObjects;
         _data.rowObjects = data.rowObjects;
         noSortRow = false;
         registerCtxIfHiddenObjPresent( data );
     }
     ...
     if( noSortCol ) {
         ...
         appCtxService.ctx.MatrixContext.parentCols = data.colObjects;
         _data.colObjects = data.colObjects;
         noSortCol = false;
         registerCtxIfHiddenObjPresent( data );
     }
  ...
  *
  */
 export const attachSortingHandler = data => {
     data.applySort = function( type ) {
         if( sortOn === 'row' ) {
             rowSort = true;
             rowSortType = type;
 ...
 const applyRowGlobalSort = ( sortedResult, returnData = false ) => {
     ...
     appCtxService.ctx.MatrixContext.parentRows = _data.rowObjects;
 ...
 const applyColGlobalSort = ( sortedResult, returnData = false ) => {
     ...
     appCtxService.ctx.MatrixContext.parentCols = _data.colObjects;
     processData( _data, appCtxService.ctx );
 };
 ...
 */
 ...
 export default exports = {
     ...
     showDeleteAllTracelinkTMWarning,
     callHideRow,
     hideRowOrColumn,
     createPopupData,
     createFacetProp,
