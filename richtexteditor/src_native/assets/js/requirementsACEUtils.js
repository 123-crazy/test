    ...
    if ( !data.hideTracelink ) {
        ...
        if ( hasTracelink || isDerived || isBasedOn ) {
            ...
            if ( hasTracelink && modelObject.props.awb0IsSuspect && modelObject.props.awb0IsSuspect.dbValues[0] === '1' ) {    // show suspect icon if suspect flag is true
                hasTracelinkElement.innerHTML = _getImageIconElement( 'indicatorSuspectLink16.svg', localeTextBundle.indicatorSuspectLink );
            } else {
                hasTracelinkElement.innerHTML = _getImageIconElement( 'indicatorTraceLink16.svg', localeTextBundle.indicatorTraceLink );
            }
            ...
            tracelinkIconPlaceHolder.innerHTML = _getImageIconElement( 'cmdCreateTraceLink24.svg', localeTextBundle.createTraceLink );
            tracelinkIconPlaceHolder.classList.add( 'aw-requirement-traceLinkIconButton' );
        }
...
export let setReadOnlyForRequirement = function( data, divElement ) {
    ...
    if ( headerTypeAttribute.toUpperCase() === 'TITLE' ) {
        ...
        if( reqHeader.getElementsByClassName( 'aw-requirement-headerId' ).length > 0 && reqHeader.getElementsByClassName( 'aw-requirement-title' ).length > 0 ) {
            var hedId = reqHeader.getElementsByClassName( 'aw-requirement-headerId' )[0].innerText;
            var titleText = reqHeader.getElementsByClassName( 'aw-requirement-title' )[0].innerHTML;
            inner = hedId + titleText;  // innerHtml required for title to get correct title in case of title contains <, > characters
        }
        var new_html = '';
...
var _getImageIconElement = function( fileName, altText ) {
    ...
    return '<img class="aw-base-icon" src="' + imgSrc + '" alt="' + altText + '" />';
};
