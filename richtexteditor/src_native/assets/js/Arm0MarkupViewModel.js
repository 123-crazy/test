...
export function clearMarkupList( isCk5 ) {
    ...
    if( !isCk5 && swaCtx.secondaryXrtPageID !== 'tc_xrt_summary_table' || widePanelCtx ) {
        // remove all markups
...
export function toggleGroup( group, inStamps ) {
    ...
    var widePanelCtx = appCtxSvc.getCtx( 'Arm0SingleRequirementWidePanelEditorActive' );
    if( !inStamps && swaCtx.secondaryXrtPageID !== 'tc_xrt_summary_table' || widePanelCtx ) {
        markupRequirement.showAll();
    }
    ...
    if ( editor && response && response.result ) {
        ...
        users.forEach( function( user ) {
            ...
            if( !user.userObj ) {
                userObject.id = user.userid;
                userObject.name = user.username;
                userObject.avatar = app.getBaseUrlPath() + '/image/typePerson48.svg';
