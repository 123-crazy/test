...
/**
 * Helper method to validate input search creteria for multiple PSVM SOA call
 * This is to avoid multiple PSVM SOA call for same search input.
 *
 * @param {*} ctx  - The application context
 * @param {*} searchInput - search input
 * @returns  true - if search input is same as earlier PSVM SOA call else false
 */
 let isSameInputSearchCriteria = function( ctx, searchInput ) {
    var isSameSearchInput = false;

    if( ctx.inputSearchCriteria ) {
        var searchInputCreteria = {
            maxToLoad: ctx.inputSearchCriteria.maxToLoad ? ctx.inputSearchCriteria.maxToLoad : '',
            maxToReturn: ctx.inputSearchCriteria.maxToReturn ? ctx.inputSearchCriteria.maxToReturn : '',
            providerName: ctx.inputSearchCriteria.providerName ? ctx.inputSearchCriteria.providerName : '',
            searchCriteria: ctx.inputSearchCriteria.searchCriteria ? ctx.inputSearchCriteria.searchCriteria : '',
            searchFilterFieldSortType: ctx.inputSearchCriteria.searchFilterFieldSortType ? ctx.inputSearchCriteria.searchFilterFieldSortType : '',
            searchFilterMap6: ctx.inputSearchCriteria.searchFilterMap6 ? ctx.inputSearchCriteria.searchFilterMap6 : '',
            searchSortCriteria: ctx.inputSearchCriteria.searchSortCriteria ? ctx.inputSearchCriteria.searchSortCriteria : '',
            cursor:
            {
                startIndex:  ctx.inputSearchCriteria.cursor  && ctx.inputSearchCriteria.cursor.startIndex !== undefined  ?  ctx.inputSearchCriteria.cursor.startIndex : ''
            }

        };

        if( _.isEqual( searchInputCreteria, searchInput ) ) {
            isSameSearchInput = true;
        }
    }

    return isSameSearchInput;
};
...
export let loadListData = function( searchInput ) {
    ...
    if( isSameInputSearchCriteria( ctx, searchInput ) && !ctx.mruObjects ) {
        return;
    }
    ctx.inputSearchCriteria = searchInput;
 ...
 */
export let loadMRUData = function( dataProvider ) {
    var ctx = appCtxService.getCtx( 'clsLocation' );
    var objects = [];
    _.forEach( ctx.standAloneObjects, function( object ) {
        ...
        if ( type === ctx.selectedClass.uid || object.classId === ctx.selectedClass.uid ||
    ...
    if ( objects.length === 0 ) {
        ctx.noSearchResultsFound = getNoResultsMsg( ctx.selectedClass );
    }
    ctx.mruObjects = objects;
    dataProvider.update( objects, objects.length );
    // update breadcrumb
    var searchCtx = appCtxService.getCtx( 'search' );
    searchCtx.totalFound = objects.length;
    appCtxService.updateCtx( 'search', searchCtx );
    eventBus.publish( 'refreshBreadCrumb', {
        searchFilterCategories: searchCtx.filterCategories,
        searchFilterMap: searchCtx.filterMap
    } );
};
 ...
 */
export let loadMRUObjects = function(  ) {
    var ctx = appCtxService.getCtx( 'clsLocation' );
    if ( !ctx.mruActive ) {
        ctx.mruActive = true;
...
export let getSearchSortCriteria = function( sortCriteria ) {
     ...
     */
    if( appCtxService.ctx.clsLocation.startClassSearch ) {
        return [];
    }
