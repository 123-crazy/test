...
export let showHtmlCompareReport = function( htmlData, data ) {
    ...
    if ( divs && divs.length > 0 ) {
        ...
        bodyTextDiv.innerHTML = compareUtils.addCssInContents( datahtml );
...
export let preProcessSpecContents = function( data, isReqSpecVersionHistory ) {
    ...
    data.mapOfVerAndRev = mapOfVerAndRev;
...
export let renderCompareContent = function( vmo ) {
    setTimeout( () => {
        ...
        if( contentElement ) {
            contentElement.innerHTML = compareUtils.addCssInContents( vmo.props[ 9 ].propValues[ 0 ] );
        }
    }, 100 );
};
