...
/**
 *
 * @module js/Qam0searchTemplateUtils
 */
...
export let callGetCategories = function( response, contextVal = true ) {
    ...
    //Remove filters for Quality Action Status and Type
    if( categoriesWithoutExpansionRetention !== undefined && categoriesWithoutExpansionRetention.length > 0 ) {
        var categoryIndex = _.findIndex( categoriesWithoutExpansionRetention, function( category ) {
            return category.internalName === 'WorkspaceObject.object_type';
        } );
        categoriesWithoutExpansionRetention.splice( categoryIndex, 1 );
        var categoryIndexForStatus = _.findIndex( categoriesWithoutExpansionRetention, function( category ) {
            return category.internalName === 'Qam0QualityAction.qam0QualityActionStatus';
        } );
        categoriesWithoutExpansionRetention.splice( categoryIndexForStatus, 1 );
        // data.categories.refineCategories = data.categories;
        // var categoryStatusResultIndex = _.findIndex( ctx.searchIncontextInfo.searchResultFilters, function( category ) {
        //     return category.searchResultCategoryInternalName === 'Qam0QualityAction.qam0QualityActionStatus';
        // } );
        // ctx.searchIncontextInfo.searchResultFilters.splice( categoryStatusResultIndex, 1 );
    }
...
/**
 * @returns {boolean} - checks if Template found
 */
export let isTemplateFound = function( response ) {
    return response.totalFound === 0;
};
 ...
 *
 * @memberof NgServices
 * @member Qam0searchTemplateUtils
 */
app.factory( 'Qam0searchTemplateUtils', () => exports );
