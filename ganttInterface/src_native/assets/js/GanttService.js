...
export let initializeProcessorClass = ( data, declViewModel, deferred ) => {
    ...
    let depModules = [ ganttProcessorPath ];
...
let getGridOptions = () => {
    ...
    for( var i = 0; i < tables.length; ++i ) {
        let attr = tables[ i ].getAttribute( 'gridid' );
        if( attr && attr === 'scheduleNavigationTree' ) {
            matchingElement = tables[ i ];
            break;
        }
    }
    let viewModel = viewModelService.getViewModelUsingElement(matchingElement );
    if( viewModel ) {
...
let assureCSSInitialization = () => {
    // This is check for CSS link which is used for gantt chart
    // styling .
    let cssCheck = $( 'head:first > link' ).filter(
 ...
 */
export let setupDates = ( declViewModel ) => {
...
/**
 * The factory to create the service for aw Gantt.
 *
 */
app.factory( 'GanttService', () => exports );
