    ...
    var onTaskClick = timelineManager.getGanttInstance().attachEvent(
        ...
        function( id, e ) {
            ...
            if( target.className.indexOf( 'gantt_tooltip_open_icon' ) !== -1 || target.className.indexOf( 'gantt_tree_open_icon' ) !== -1 ) {
                ...
                eventBus.publish( 'updateSWAForEvent', appCtx.ctx.locationContext.modelObject );
                return false;
            }
            // Publish event for setup the Program Board
            eventBus.publish( 'setupProgramBoard.selectionChanged', id );
...
/**
 * Refresh rollup of parent
 * @param {String} id The Task Id to rollup
 */
function refreshRollupParents( id ) {
    var task = timelineManager.getGanttInstance().getTask( id );
    if( !task.rollup ) {
        return;
    }
    timelineManager.getGanttInstance().eachParent( function( parent ) {
        timelineManager.getGanttInstance().refreshTask( parent.id );
    }, id );
}
...
export let registerAWEvents = function( timelineDataSource ) {
    ...
    _awEvents.push( eventBus.subscribe( 'planNavigationTree.plTable.resizeCheck', function() {
        resizer();
    } ) );
...
export let getComputedHeight = function() {
    if( appCtx.ctx.locationContext['ActiveWorkspace:SubLocation'] === 'com.siemens.splm.client.prgplanning:PlanNavigationSubLocation' ) {
        return _planNavTreeUtils.getTreeTableHeight();
    }
