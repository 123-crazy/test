...
export let setTimelineConfig = function( timelineConfig, timelineZoomPreference ) {
    ...
    let viewType = 'month';
    var contextObj = appCtx.ctx.locationContext.modelObject;
...
export let setTimelineZoomLevel = function( viewType, flag ) {
    ...
    switch ( viewType ) {
        ...
        case 'unit_of_time_measure':
            var contextObj = appCtx.ctx.locationContext.modelObject;
            if( contextObj.props.prg0UnitOfTimeMeasure ) {
                viewType = getTimelineScaleMapping( contextObj.props.prg0UnitOfTimeMeasure.dbValues[ 0 ] );
            } else {
...
let getTimelineRowHeight = function() {
    ...
    let rowHeight = appCtx.ctx.layout === 'compact' ? _t.Const.HEIGHT_COMPACT_ROW : _t.Const.HEIGHT_ROW;
    return rowHeight + 1;
};
 ...
 * @param {data} data
 */
export let updateSelectionInPopup = function( data ) {
 ...
 * @param {data} data
 */
export let showHighlightedEventInPopup = ( data ) => {
    let selection = appCtx.ctx.selected.uid;
    let vmc = data.dataProviders.listDataProvider.viewModelCollection;
    var index = vmc.findViewModelObjectById( selection );
    if( index > -1 ) {
        var vmo = vmc.getViewModelObject( index );
        data.dataProviders.listDataProvider.selectionModel.setSelection( vmo );
    }
};
