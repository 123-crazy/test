...
export let registerAWEvents = function( timelineDataSource ) {
    if( _schNavTreeUtils.isScheduleNavigationSublocation() ) {
        // Sync Tree table resizing
        _awEvents.push( eventBus.subscribe( 'scheduleNavigationTree.plTable.resizeCheck', function() {
            updateGanttHeight();
        } ) );
...
export let updateGanttHeight = function() {
    var heightInPixels = exports.getComputedHeight() + 'px';
    var ganttElement = document.getElementsByClassName( 'smgantt' );
    if( ganttElement && ganttElement[ 0 ].style.height !== heightInPixels ) {
        ganttElement[ 0 ].style.height = heightInPixels;
...
export let getComputedHeight = function() {
    let height = 0;
    if( _schNavTreeUtils.isScheduleNavigationSublocation() ) {
        height = _schNavTreeUtils.getTreeTableHeight() + 2;
    } else {
        ...
        height = subLocHeight - columnHeight - filterHeight;
        if( height < 200 ) {
            height = 200;
        }
    }
