        ...
        if( queryId && ptn_scheme_uid !== null ) {
            ...
            if( isHandled ) {
                ...
                if( ptn_scheme_uid === '' ) {
                    ...
                    _appCtxSvc.updatePartialCtx( 'aceActiveContext.context.requestPref', requestPrefValue );
                }
                ...
                _appCtxSvc.updatePartialCtx( 'aceActiveContext.context.configContext', value );
...
var getSelectionFromPCISilently = function( inputDseUid ) {
    ...
    var elementToPCIMap = _appCtxSvc.ctx.aceActiveContext.context.elementToPCIMap;
...
var configContextChanged = function( isSchemeChange ) {
    ...
    if( _appCtxSvc.ctx && _appCtxSvc.ctx.aceActiveContext && _appCtxSvc.ctx.aceActiveContext.context ) {
        wse_uid = _appCtxSvc.ctx.aceActiveContext.context.topElement.props.awb0UnderlyingObject.dbValues[ 0 ];
        if( _appCtxSvc.ctx.aceActiveContext.context.productContextInfo.props.awb0Product ) {
            if( _appCtxSvc.ctx.aceActiveContext.context.productContextInfo.props.awb0Product.dbValues[ 0 ] !== null &&
                _appCtxSvc.ctx.aceActiveContext.context.productContextInfo.props.awb0Product.dbValues[ 0 ] !== wse_uid ) {
                dse_uid = _appCtxSvc.ctx.aceActiveContext.context.productContextInfo.props.awb0Product.dbValues[ 0 ];
            }
        }

        if( _appCtxSvc.ctx.aceActiveContext.context.productContextInfo.props.fgf0PartitionScheme ) {
            if( _appCtxSvc.ctx.aceActiveContext.context.productContextInfo.props.fgf0PartitionScheme.dbValues[ 0 ] !== null ) {
                ptn_scheme_uid = _appCtxSvc.ctx.aceActiveContext.context.productContextInfo.props.fgf0PartitionScheme.dbValues[ 0 ];
                ptn_scheme_name = _appCtxSvc.ctx.aceActiveContext.context.productContextInfo.props.fgf0PartitionScheme.uiValues[ 0 ];
            }
        }
    }
...
export let reconfigureViewerToHost = function() {
    ...
    declUtils.loadDependentModule( 'js/occmgmtBackingObjectProviderService' )
        .then( ( occMBOPSMod ) => {
            if( occMBOPSMod ) {
                return occMBOPSMod.getBackingObjects( [ appCtxService.getCtx( 'aceActiveContext.context.rootElement' ) ] );
            }
        } ).then( ( topLinesArray ) => {
...
GetAppliedPartitionSchemeHandler.prototype.handleQuery = function( inputMessage ) {
    if( !_.isEmpty( inputMessage.queryData ) ) {
        ...
        if( queryId && Boolean( getQueryHandler( queryId ) ) ) {
...
var isWorksetOpen = function() {
    if( _appCtxSvc.ctx && _appCtxSvc.ctx.aceActiveContext && _appCtxSvc.ctx.aceActiveContext.context ) {
        if( _appCtxSvc.ctx.aceActiveContext.context.topElement && _appCtxSvc.ctx.aceActiveContext.context.topElement.props.awb0UnderlyingObject ) {
            var underlyingObject = _cdm.getObject( _appCtxSvc.ctx.aceActiveContext.context.topElement.props.awb0UnderlyingObject.dbValues[ 0 ] );
    ...
    if( !_.isEmpty( dataObjects ) ) {
        ...
        if( queryId ) {
            ...
            if( isHandled ) {
                ...
                declUtils.loadDependentModule( 'js/Cm1ChangeCommandService', AwPromiseService.instance, AwInjectorService.instance )
...
app.factory( 'hostVisQueryService', () => exports );
