...
export let reloadAndSelectProjectTeamTreeNode = function( eventData ) {
    ...
    if( eventData.isStatusChange ) {
        ...
        for( var i = 0; i < addedObjects.groupRoles.length; i++ ) {
            vmoIdList.push( projectId + '_' + addedObjects.groupRoles[i].tcGroup.uid + '_' + addedObjects.groupRoles[i].tcRole.uid );
        }
        ...
        if( addedObjects.userParentToSelect ) {
            ...
            vmoIdList.push( projectId + '_' + addedObjects.userParentToSelect[i] );
            }
        }
    }
...
export let getInputParentNode = function( treeLoadInput, data ) {
    ...
    if( !isFiltering || isFiltering && isNodeExpanded  ) {
        if( rootNode.grp ) {
            group = {
                type: rootNode.grp.type,
                uid: rootNode.grp.uid
            };
        }
 ...
 * @returns vmNode
 */
export let processGroupOrRoleNode = function( groupRoleNode, type, isRemovable, levelNdx, childNdx, isExpanded ) {
var displayName = '';
var objUid = '';
var iconURL = '';
var vmNode = '';
var vmNodeType = '';
//groupRoleNode.grp.type will either be 'Group' or 'ProjectTeam' in case the node is group node or role node
if( groupRoleNode.role.type === 'unknownType' && groupRoleNode.grp.type !== 'unknownType' ) {
    displayName = groupRoleNode.grp.props.object_full_name.uiValues[ 0 ];
    objUid = appCtxService.ctx.xrtSummaryContextObject.uid + '_' + groupRoleNode.grp.uid;
    iconURL = app.getBaseUrlPath() + '/image/typeProjectTeam48.svg';
    vmNodeType = 'Group';
} else if( groupRoleNode.role.type === 'Role' ) {
    displayName = groupRoleNode.grp.props.object_full_name.uiValues[ 0 ] + '.' + groupRoleNode.role.props.role_name.uiValues[ 0 ];
    //isRemovable to know give unique id to direct role - dba.DBA or the role is under the dba group - 'dba.DBA'
    objUid = appCtxService.ctx.xrtSummaryContextObject.uid + '_' + groupRoleNode.grp.uid + '_' + groupRoleNode.role.uid + '_' + isRemovable.toString();
    iconURL = app.getBaseUrlPath() + '/image/typeRole48.svg';
    vmNodeType = 'Role';
}
if( objUid !== '' ) {
    ...
    vmNode.isLeaf = false;
    vmNode.grp = groupRoleNode.grp;
    vmNode.role = groupRoleNode.role;
    vmNode.isRemovable = isRemovable;
    vmNode.props = {};
    vmNode.props.status = {
        value: '',
        uiValue: ''
 ...
 */
export let processGroupsAndRoles = function( groupsAndRoles, treeLoadInput, serviceData, childNdx, levelNdx, isFiltering, parentNode, vmNodes ) {
    ...
    // In other words, do not execute if block if the SOA input parent Node is same as SOA response groupRoleNode.
        if( parentNode.uid === 'top' || !( parentNode.grp.uid === groupRoleNode.grp.uid && parentNode.role.uid === groupRoleNode.role.uid ) ) {   // tHis should be executed when we are loading the first level tree node structure & during filtering
            ...
            var firstLevelNode = exports.processGroupOrRoleNode( groupRoleNode, node.type, node.isRemovable, newLevelNdx, childNdx, isExpanded );
            if( firstLevelNode ) {
        ...
        if( childMembers.length > 0 ) {
            for ( var childMemberNdx = 0; childMemberNdx < childMembers.length; childMemberNdx++ ) {
                ...
                var vmNode = awTableSvc
                    .createViewModelTreeNode( objUid, objTypeDbValue, displayName, newLevelNdx, childMemberNdx, iconURL );
                vmNode.isLeaf = true;
                vmNode.props = {};
                vmNode.props.status = {
        ...
        if( childGroupsAndRoles.length > 0 ) {
            exports.processSearchResults( childGroupsAndRoles, treeLoadInput, serviceData, newLevelNdx, isFiltering, parentNode, vmNodes, true );
        }
};
 ...
 */
export let processSearchResults = function( groupsAndRoles, treeLoadInput, serviceData, levelNdx, isFiltering, parentNode, vmNodes, isProcessingChild ) {
    var newLevelNdx = levelNdx;
    ...
    */
     var processChildMembers = true;
     if( parentNode.type === 'Role' && parentNode.levelNdx === 0 ) {
        processChildMembers = groupsAndRoles[childNdx].groupOrRole.grp.uid === parentNode.grp.uid && groupsAndRoles[childNdx].groupOrRole.role.uid === parentNode.role.uid;
     }

     if( !processChildMembers ) {
         //skip processing structure if response does not belongs to the parent node
        continue;
     }
    /*
    Consider scenario where -
    ...
    */
    if( !isProcessingChild && isFiltering  && !( groupsAndRoles[childNdx].groupOrRole.grp.type === 'unknownType' && groupsAndRoles[childNdx].groupOrRole.role.type === 'unknownType' ) ) // group & role type is same , means grp & role is of unknownType
        {
        ...
        exports.processGroupsAndRoles( groupsAndRoles[childNdx], treeLoadInput, serviceData, childNdx, newLevelNdx, isFiltering, parentNode, vmNodes );
    }
};
 ...
 */
export let loadRootNode = function( searchResults, treeLoadInput, data ) {
...
treeLoadInput.parentNode.totalFound = searchResults.totalCount;
...
if( isNodeExpanded ) { //parentNode._expandRequested ) {
    isFiltering = false;
}
if( isFiltering ) {
...
exports.processSearchResults( searchResults.groupsAndRoles, treeLoadInput, searchResults.ServiceData, levelNdx, isFiltering, parentNode, vmNodes );

//Build the treeLoadResult based on nodes created
var treeLoadResult = awTableSvc.buildTreeLoadResult( treeLoadInput, vmNodes, true, startReachedVar,
