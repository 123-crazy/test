...
export let addObjectToSearchFolderInt = function( searchFoldersInputArr, isAdd ) {
    ...
    soaService.post( 'Internal-Search-2020-12-SearchFolder', 'createOrEditSearchFolders', {
        ...
        function( response ) {
            if( isAdd ) {
                let refresh = appCtxService.ctx.locationContext['ActiveWorkspace:SubLocation'] === 'showObject';
                if ( !refresh ) {
                ...
                exports.displayConfirmation( response );
    ...
    for( var i = 0; i < attributeValues.length; i++ ) {
        ...
        if ( _.startsWith( attribute, '##' ) ) {
            ...
            if( multipleValues && multipleValues.length > 0 && attributeValues[i + 1].indexOf( INTERNAL_KEYWORD ) !== -1 ) {
                filterValue = exports.splitInternalAndDisplayNameUsingInternalKeyword( multipleValues, true );
            } else {
