 ...
 *
 * @module js/Caw0AnalysisTemplateService
 */
import app from 'app';
...
var NODE_TEMPLATE_NAME = 'Caw0TileNodeTemplate';
var DEFAULT_NODE_TEMPLATE_NAME = 'Caw0DefaultTileNodeTemplate';
...
var getTemplateContent = function( templateId, baseTemplateString, propertyNames ) {
    ...
    if( propertyNames instanceof Array ) {
        ...
        if( len > 1 ) {
            templateData.property_list = propertyNames.slice( 1 );
        }
 ...
 */
var getDefaultTemplateContent = function( templateId, baseTemplateString, propertyNames ) {
    var templateData = {
        template_id: null,
        title: null,
        title_editable: null,
 ...
 */
export let getNodeTemplate = function( nodeTemplateCache, propertyNames, isGroup, useMultiLevelTemplate, nodeType ) {
    //template doesn't exist, construct it and put in template cache
    var baseTemplateId = nodeType === 'CauseGroup' ? 'Caw0IshikawaCauseNodeTemplate' : NODE_TEMPLATE_NAME;
...
/**
 * Get node template by populate the base template with given binding property names for relation tab
 */
export let getDefaultNodeTemplate = function( nodeTemplateCache, propertyNames, isGroup, useMultiLevelTemplate ) {
    //template doesn't exist, construct it and put in template cache
    var baseTemplateId = DEFAULT_NODE_TEMPLATE_NAME;
    ...
    var newTemplate = _.cloneDeep( baseTemplate );
    newTemplate.templateId = templateId;
    newTemplate.templateContent = getDefaultTemplateContent( templateId, baseTemplate.templateContent, propertyNames );
...
export let getBindPropertyNames = function( nodeObject ) {
    ...
    if( nodeObject.modelType && nodeObject.modelType.typeHierarchyArray.indexOf( 'CAW05Why' ) > -1 ) {
        properties.push( 'Name' );
        properties.push( 'Problem Definition' );
    } else if( nodeObject.modelType && ( nodeObject.modelType.typeHierarchyArray.indexOf( 'CAW0Defect' ) > -1 || nodeObject.modelType.typeHierarchyArray.indexOf( 'CAW0Ishikawa' ) > -1 ) ) {
        properties.push( 'Name' );
        properties.push( 'Description' );
    } else if( nodeObject && nodeObject.type === 'CauseGroup' ) {
        properties.push( 'Name' );
    }
...
export let getBindProperties = function( nodeObject, isRoot ) {
    ...
    if( nodeObject && nodeObject.props && nodeObject.modelType && nodeObject.modelType.typeHierarchyArray.indexOf( 'CAW05Why' ) > -1 ) {
        var propsArray = [ 'object_name', 'caw0ProbDefinition' ];

        for( var i = 0; i < propsArray.length; i++ ) {
            var nameValue = nodeObject.props[ propsArray[ i ] ];
            properties[ nameValue.propertyDescriptor.displayName ] = nameValue.uiValues[ 0 ];

            if( i === 0 ) {
                properties.Title = nameValue.propertyDescriptor.displayName;
                properties[ nameValue.propertyDescriptor.displayName ] = 'Problem Definition';
            }
        }
    }
    if( nodeObject && nodeObject.props && nodeObject.modelType && nodeObject.modelType.typeHierarchyArray.indexOf( 'CAW0Ishikawa' ) > -1 ) {
        properties.Title = 'Name';
        properties.Name = nodeObject.props.caw0ProbDefinition.uiValues[ 0 ];
        properties.Description = nodeObject.props.object_desc.uiValues[ 0 ];
    }

    if( nodeObject && nodeObject.props && nodeObject.modelType && nodeObject.modelType.typeHierarchyArray.indexOf( 'CAW0Defect' ) > -1 ) {
        properties.Title = 'Name';
        properties.Name = nodeObject.props.caw0Context.dbValues[ 0 ] === 'Ishikawa' ? nodeObject.props.object_name.uiValues[ 0 ] : nodeObject.props.caw0WhySequence.uiValues[ 0 ];
        properties.Description = nodeObject.props.caw0Context.dbValues[ 0 ] === 'Ishikawa' ? nodeObject.props.object_desc.uiValues[ 0 ] : nodeObject.props.object_name.uiValues[ 0 ];
    }

    if( nodeObject && nodeObject.props && nodeObject.type === 'CauseGroup' ) {
        properties.Title = 'Name';
        properties.Name = nodeObject.props.object_name.uiValues[ 0 ];
        properties.Description = nodeObject.props.object_name.uiValues[ 0 ];
    }
    ...
    //set NodeObject type
    properties.node_object_type = nodeObject.type;
...
/**
 * Get the binding properties for the given node object in default Relation tab
 *
 * @param nodeObject the node model object
 * @param propertyNames the names of node object property to display
 * @return the object including all the required binding properties for a node template
 */
export let getBindPropertiesForRelation = function( nodeObject, isRoot ) {
    var properties = {};
