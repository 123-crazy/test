 ...
 *
 *
 * @module js/CAW0GraphDiagramSVGUtilService
 */
import app from 'app';
...
var updateStyles = function( text, rect, svgTag, path ) {
    ...
    for( i = len - 1; i >= 0; i-- ) {
        ...
        var fillColorText = "rgb(30, 30, 30)";
...
export let getImagesIconElement = function( xmlDoc ) {
    ...
    imageElements.forEach( imageElement => {
        if( imageElement.attributes[ 'xlink:href' ].value.split( '/' )[ 0 ] === 'http:' ) {
            var objectType = imageElement.parentNode.classList.value;
            var iconID;
            switch ( objectType ) {
                case 'CAW05Why':
                    iconID = 'typeFiveWhy48';
                    break;
                case 'CAW0Ishikawa':
                    iconID = 'typeIshikawaObject48';
                    break;
                case 'CAW0Defect':
                    iconID = 'typeDefectiveObject48';
                    break;
            }
            imageElement.attributes[ 'xlink:href' ].value = app.getBaseUrlPath() + '/image/' + iconID + '.svg';
        }
    } );
...
export let processUseTag = function( svgRoot, imagesInfo ) {
    ...
    _.forEach( imagesInfo, function( imageInfo, key ) {
        var useTag = getCreateUseElementTag( imageInfo );
        var iconContent = getSvgIconContent( imageInfo );
        //replace linear-gradient id with unique to resolve confilicts
        iconContent.innerHTML = iconContent.innerHTML.replace( /linear-gradient/gi, 'linear-gradient-' + key );
...
app.factory( 'CAW0GraphDiagramSVGUtilService', () => exports );
